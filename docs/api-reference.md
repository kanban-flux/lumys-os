...
Lumys OS exposes a REST API, WebSocket endpoints, and SSE streaming when the daemon is running. The default listen address is `http://127.0.0.1:4200`.

All responses include security headers (CSP, X-Frame-Options, X-Content-Type-Options, HSTS) and are protected by a GCRA cost-aware rate limiter with per-IP token bucket tracking and automatic stale entry cleanup. Lumys OS implements 16 security systems including Merkle audit trails, taint tracking, WASM dual metering, Ed25519 manifest signing, SSRF protection, subprocess sandboxing, and secret zeroization.
  ...
  "templates": [
    {
      "name": "hello-world",
      "description": "A friendly greeting agent",
      "path": "/home/user/.lumys/agents/hello-world/agent.toml"
    },
    {
      "name": "coder",
      "description": "Expert coding assistant",
      "path": "/home/user/.lumys/agents/coder/agent.toml"
    }
  ],
  "total": 30
}
...
List OFP (Lumys OS Protocol) wire peers and their connection status.
...
Lumys OS maintains a built-in catalog of 51+ models across 20 providers. These endpoints allow you to browse available models, check provider authentication status, and resolve model aliases.
...
```json
{
  "status": "created",
  "skill": "my-skill",
  "path": "/home/user/.lumys/skills/my-skill"
}
```
...
Install a skill from ClawHub. Downloads, verifies SHA256 checksum, scans for prompt injection, and converts SKILL.md format to Lumys OS skill.toml automatically.
...
Lumys OS supports both Model Context Protocol (MCP) for tool interoperability and Agent-to-Agent (A2A) protocol for cross-system agent communication.
...
MCP HTTP transport endpoint. Accepts JSON-RPC 2.0 requests and exposes Lumys OS tools via the MCP protocol to external clients.
...
{
  "name": "Lumys OS",
  "description": "Lumys OS Agent Operating System",
...
Lumys OS maintains a Merkle hash chain audit trail for all security-relevant operations. These endpoints allow inspection and verification of the audit log integrity.
...
Lumys OS exposes an OpenAI-compatible API for drop-in integration with tools that support the OpenAI API format (Cursor, Continue, Open WebUI, etc.).
...
**Model resolution** (the `model` field maps to an Lumys OS agent):

| Format | Example | Behavior |
|--------|---------|----------|
| `lumys:<name>` | `lumys:coder` | Find agent by name |
