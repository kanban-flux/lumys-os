//! Load & performance tests for the Lumys OS API.
//!
//! Measures throughput under concurrent access: agent spawning, API endpoint
//! latency, session management, and memory usage.
//!
//! Run: cargo test -p lumys-api --test load_test -- --nocapture

use axum::Router;
use lumys_api::middleware;
use lumys_api::routes::{self, AppState};
use lumys_kernel::LumysKernel;
use lumys_types::config::{DefaultModelConfig, KernelConfig};
use std::sync::Arc;
...
async fn load_metrics_sustained() {
    ...
    for _ in 0..n {
        ...
        assert!(body.contains("lumys_agents_active"));
    }
