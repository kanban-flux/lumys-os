#!/usr/bin/env bash
# Lumys OS installer — works on Linux, macOS, WSL
# Usage: curl -sSf https://lumys.sh | sh
#
# Environment variables:
#   LUMYS_INSTALL_DIR  — custom install directory (default: ~/.lumys/bin)
#   LUMYS_VERSION      — install a specific version tag (default: latest)

set -euo pipefail

REPO="ENI-Ethereal-Nexus-Institute/lumys-os"
INSTALL_DIR="${LUMYS_INSTALL_DIR:-$HOME/.lumys/bin}"
...
detect_platform() {
    ...
    case "$OS" in
        ...
        mingw*|msys*|cygwin*)
            ...
            echo "    irm https://lumys.sh/install.ps1 | iex"
            echo ""
            echo "  Or download the .msi installer from:"
            echo "    https://github.com/$REPO/releases/latest"
            echo ""
            echo "  Or install via cargo:"
            echo "    cargo install --git https://github.com/$REPO lumys-cli"
            exit 1
            ;;
...
install() {
    ...
    echo ""
    echo "  Lumys OS Installer"
    echo "  =================="
    echo ""

    # Get latest version
    if [ -n "${LUMYS_VERSION:-}" ]; then
        VERSION="$LUMYS_VERSION"
    ...
    URL="https://github.com/$REPO/releases/download/$VERSION/lumys-$PLATFORM.tar.gz"
    CHECKSUM_URL="$URL.sha256"

    echo "  Installing Lumys OS $VERSION for $PLATFORM..."
    mkdir -p "$INSTALL_DIR"

    # Download to temp
    TMPDIR=$(mktemp -d)
    ARCHIVE="$TMPDIR/lumys.tar.gz"
    ...
    if [ "$OS" = "darwin" ]; then
        if command -v xattr &>/dev/null; then
            xattr -cr "$INSTALL_DIR/lumys" 2>/dev/null || true
        fi
        if command -v codesign &>/dev/null; then
            if ! codesign --force --sign - "$INSTALL_DIR/lumys"; then
                echo ""
                echo "  Warning: ad-hoc code signing failed."
                echo "  On Apple Silicon, the binary may be killed (SIGKILL) by Gatekeeper."
                echo "  Try manually: xattr -cr $INSTALL_DIR/lumys && codesign --force --sign - $INSTALL_DIR/lumys"
                echo ""
            fi
        fi
    fi
    ...
    if [ -n "$SHELL_RC" ] && ! grep -q "lumys" "$SHELL_RC" 2>/dev/null; then
    ...
    # Verify installation
    if "$INSTALL_DIR/lumys" --version >/dev/null 2>&1; then
        INSTALLED_VERSION=$("$INSTALL_DIR/lumys" --version 2>/dev/null || echo "$VERSION")
        echo ""
        echo "  Lumys OS installed successfully! ($INSTALLED_VERSION)"
    else
        echo ""
        echo "  Lumys OS binary installed to $INSTALL_DIR/lumys"
    fi
