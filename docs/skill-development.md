...
Skills are pluggable tool bundles that extend agent capabilities in Lumys OS. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to FangHub, and CLI management.
...
Skills are installed to `~/.lumys/skills/` and made available to agents through the skill registry. Lumys OS ships with **60 bundled skills** that are compiled into the binary and available immediately.
...
Lumys OS includes 60 expert knowledge skills compiled into the binary (no installation needed):
...
1. Lumys OS sends a JSON payload to the script's stdin:
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Lumys OS-Skill/1.0"})
...
### Using the Lumys OS Python SDK

For more advanced skills, use the Python SDK (`sdk/python/lumys_sdk.py`):

```python
#!/usr/bin/env python3
from lumys_sdk import SkillHandler
...
```bash
lumys skill install /path/to/my-skill
```

This reads the `skill.toml`, validates the manifest, and copies the skill to `~/.lumys/skills/my-skill/`.

### From FangHub

```bash
lumys skill install web-summarizer
```
...
```bash
lumys skill install https://github.com/user/lumys-skill-example.git
```
...
```bash
lumys skill remove web-summarizer
```
...
FangHub is the community skill marketplace for Lumys OS.
...
```bash
lumys skill search "web scraping"
```
...
Lumys OS can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.

### Automatic Conversion

```bash
lumys skill install /path/to/openclaw-skill
```

If the directory contains an OpenClaw-style skill (Node.js package), Lumys OS:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to Lumys OS conventions.
4. Copies the skill to the Lumys OS skills directory.
...
```bash
lumys skill install /path/to/skill-directory
```

Skills imported via `lumys migrate --from openclaw` are also scanned and reported in the migration report, with instructions for manual reinstallation.
