//! Channel bridge wiring — connects the Lumys OS kernel to channel adapters.
//!
//! Implements `ChannelBridgeHandle` on `LumysKernel` and provides the
//! `start_channel_bridge()` entry point called by the daemon.

use lumys_channels::bridge::{BridgeManager, ChannelBridgeHandle};
use lumys_channels::discord::DiscordAdapter;
use lumys_channels::email::EmailAdapter;
use lumys_channels::google_chat::GoogleChatAdapter;
use lumys_channels::irc::IrcAdapter;
use lumys_channels::matrix::MatrixAdapter;
use lumys_channels::mattermost::MattermostAdapter;
use lumys_channels::rocketchat::RocketChatAdapter;
use lumys_channels::router::AgentRouter;
use lumys_channels::signal::SignalAdapter;
use lumys_channels::slack::SlackAdapter;
use lumys_channels::teams::TeamsAdapter;
use lumys_channels::telegram::TelegramAdapter;
use lumys_channels::twitch::TwitchAdapter;
use lumys_channels::types::ChannelAdapter;
use lumys_channels::whatsapp::WhatsAppAdapter;
use lumys_channels::xmpp::XmppAdapter;
use lumys_channels::zulip::ZulipAdapter;
// Wave 3
use lumys_channels::bluesky::BlueskyAdapter;
use lumys_channels::feishu::FeishuAdapter;
use lumys_channels::line::LineAdapter;
use lumys_channels::mastodon::MastodonAdapter;
use lumys_channels::messenger::MessengerAdapter;
use lumys_channels::reddit::RedditAdapter;
use lumys_channels::revolt::RevoltAdapter;
use lumys_channels::viber::ViberAdapter;
// Wave 4
use lumys_channels::flock::FlockAdapter;
use lumys_channels::guilded::GuildedAdapter;
use lumys_channels::keybase::KeybaseAdapter;
use lumys_channels::nextcloud::NextcloudAdapter;
use lumys_channels::nostr::NostrAdapter;
use lumys_channels::pumble::PumbleAdapter;
use lumys_channels::threema::ThreemaAdapter;
use lumys_channels::twist::TwistAdapter;
use lumys_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use lumys_channels::dingtalk::DingTalkAdapter;
use lumys_channels::dingtalk_stream::DingTalkStreamAdapter;
use lumys_channels::discourse::DiscourseAdapter;
use lumys_channels::gitter::GitterAdapter;
use lumys_channels::gotify::GotifyAdapter;
use lumys_channels::linkedin::LinkedInAdapter;
use lumys_channels::mumble::MumbleAdapter;
use lumys_channels::ntfy::NtfyAdapter;
use lumys_channels::webhook::WebhookAdapter;
use lumys_channels::wecom::WeComAdapter;
use lumys_kernel::LumysKernel;
use lumys_types::agent::AgentId;
use std::sync::Arc;
use std::time::{Duration, Instant};
use tracing::{error, info, warn};

use lumys_runtime::str_utils::safe_truncate_str;

/// Wraps `LumysKernel` to implement `ChannelBridgeHandle`.
pub struct KernelBridgeAdapter {
    kernel: Arc<LumysKernel>,
    started_at: Instant,
}
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn send_message_with_blocks(
        ...
        let text: String = blocks
            ...
            .filter_map(|b| match b {
                lumys_types::message::ContentBlock::Text { text, .. } => Some(text.as_str()),
                _ => None,
            })
            .collect::<Vec<_>>()
            .join("\n");
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.lumys/agents/{name}/agent.toml
        ...
        let manifest: lumys_types::agent::AgentManifest =
    ...
    async fn uptime_info(&self) -> String {
        ...
        if hours > 0 {
            format!(
                "Lumys OS status: {}h {}m uptime, {} agent(s)",
                hours,
                mins,
                agents.len()
            )
            ...
            format!(
                "Lumys OS status: {}m uptime, {} agent(s)",
                mins,
                agents.len()
            )
        }
    ...
    async fn list_providers_text(&self) -> String {
        ...
        for p in catalog.list_providers() {
            let status = match p.auth_status {
                lumys_types::model_catalog::AuthStatus::Configured => "configured",
                lumys_types::model_catalog::AuthStatus::Missing => "not configured",
                lumys_types::model_catalog::AuthStatus::NotRequired => "local (no key needed)",
            };
            msg.push_str(&format!(
                "  {} — {} [{}, {} model(s)]\n",
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.lumys/skills/ or install from the marketplace.".to_string();
        }
        ...
        let result = self
            ...
            .execute_run(
                ...
                |step_agent| match step_agent {
                    lumys_kernel::workflow::StepAgent::ById { id } => {
                        let aid: AgentId = id.parse().ok()?;
                        let entry = registry_ref.get(aid)?;
                        Some((aid, entry.name.clone()))
                    }
                    lumys_kernel::workflow::StepAgent::ByName { name } => {
    ...
    async fn list_schedules_text(&self) -> String {
        ...
        for job in &jobs {
            ...
            let sched = match &job.schedule {
                lumys_types::scheduler::CronSchedule::Cron { expr, .. } => expr.clone(),
                lumys_types::scheduler::CronSchedule::Every { every_secs } => {
                    format!("every {every_secs}s")
                }
                lumys_types::scheduler::CronSchedule::At { at } => {
    ...
    async fn manage_schedule_text(&self, action: &str, args: &[String]) -> String {
        match action {
            "add" => {
                ...
                let job = lumys_types::scheduler::CronJob {
                    id: lumys_types::scheduler::CronJobId::new(),
                    agent_id: agent.id,
                    name: format!("chat-{}", &agent.name),
                    enabled: true,
                    schedule: lumys_types::scheduler::CronSchedule::Cron {
                        expr: cron_expr.clone(),
                        tz: None,
                    },
                    action: lumys_types::scheduler::CronAction::AgentTurn {
                        message: message.clone(),
                        model_override: None,
                        timeout_secs: None,
                    },
                    delivery: lumys_types::scheduler::CronDelivery::None,
    ...
    async fn channel_overrides(
        &self,
        channel_type: &str,
    ) -> Option<lumys_types::config::ChannelOverrides> {
    ...
    async fn authorize_channel_user(
        ...
        let auth_action = match action {
            "chat" => lumys_kernel::auth::Action::ChatWithAgent,
            "spawn" => lumys_kernel::auth::Action::SpawnAgent,
            "kill" => lumys_kernel::auth::Action::KillAgent,
            "install_skill" => lumys_kernel::auth::Action::InstallSkill,
            _ => lumys_kernel::auth::Action::ChatWithAgent,
        };
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            lumys_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            lumys_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
    ...
    async fn a2a_agents_text(&self) -> String {
        ...
        for (url, card) in agents.iter() {
            ...
            if !desc.is_empty() {
                let short = lumys_types::truncate_str(desc, 60);
...
fn parse_trigger_pattern(s: &str) -> Option<lumys_kernel::triggers::TriggerPattern> {
    use lumys_kernel::triggers::TriggerPattern;
...
pub async fn start_channel_bridge(kernel: Arc<LumysKernel>) -> Option<BridgeManager> {
...
pub async fn start_channel_bridge_with_config(
    ...
    if let Some(ref fs_config) = config.feishu {
        if let Some(secret) = read_token(&fs_config.app_secret_env, "Feishu") {
            let region = lumys_channels::feishu::FeishuRegion::parse_region(&fs_config.region);
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = lumys_kernel::config::load_config(Some(&config_path));
