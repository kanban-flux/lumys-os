// Lumys OS Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await Lumys OSAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await Lumys OSAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        Lumys OSToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        Lumys OSToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await Lumys OSAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        Lumys OSToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        Lumys OSToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await Lumys OSAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        Lumys OSToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        Lumys OSToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      Lumys OSToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await Lumys OSAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          Lumys OSToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          Lumys OSToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await Lumys OSAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          Lumys OSToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          Lumys OSToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        Lumys OSToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await Lumys OSAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        Lumys OSToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        Lumys OSToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      Lumys OSToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await Lumys OSAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          Lumys OSToast.success('Trigger deleted');
        } catch(e) {
          Lumys OSToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
