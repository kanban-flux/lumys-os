...
fn gateway_dir() -> PathBuf {
    lumys_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::LumysKernel>) {
    ...
    let lumys_url = format!("http://{api_listen}");
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let child = tokio::process::Command::new(node_cmd)
                ...
                .env("LUMYS_URL", &lumys_url)
                .env("LUMYS_DEFAULT_AGENT", &default_agent)
...
mod tests {
    ...
    fn test_embedded_files_not_empty() {
        ...
        assert!(GATEWAY_PACKAGE_JSON.contains("@lumys/whatsapp-gateway"));
    }
    ...
    #[test]
    fn test_gateway_dir_under_lumys_home() {
        let dir = gateway_dir();
    ...
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("lumys_test_gateway");
