...
use lumys_types::message::{ContentBlock, Message, StopReason, TokenUsage};
...
pub struct CompletionRequest {
    ...
    pub thinking: Option<lumys_types::config::ThinkingConfig>,
}
...
pub struct DriverConfig {
    ...
    /// `claude` CLI.  Defaults to `true` because Lumys OS runs as a daemon
    /// with no interactive terminal, so permission prompts would block
    /// indefinitely.  Lumys OS's own capability / RBAC layer already
