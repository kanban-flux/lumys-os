...
/// 2. `~/.lumys/agents/` (installed templates)
/// 3. `LUMYS_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    // Installed templates (respects LUMYS_HOME)
    let of_home = if let Ok(h) = std::env::var("LUMYS_HOME") {
        PathBuf::from(h)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".lumys")
    } else {
        std::env::temp_dir().join(".lumys")
    };
    {
    ...
    if let Ok(env_dir) = std::env::var("LUMYS_AGENTS_DIR") {
