//! LumysKernel — assembles all subsystems and provides the main API.
...
use lumys_memory::MemorySubstrate;
use lumys_runtime::agent_loop::{
    run_agent_loop, run_agent_loop_streaming, strip_provider_prefix, AgentLoopResult,
};
use lumys_runtime::audit::AuditLog;
use lumys_runtime::drivers;
use lumys_runtime::kernel_handle::{self, KernelHandle};
use lumys_runtime::llm_driver::{
    CompletionRequest, CompletionResponse, DriverConfig, LlmDriver, LlmError, StreamEvent,
};
use lumys_runtime::python_runtime::{self, PythonConfig};
use lumys_runtime::routing::ModelRouter;
use lumys_runtime::sandbox::{SandboxConfig, WasmSandbox};
use lumys_runtime::tool_runner::builtin_tool_definitions;
use lumys_types::agent::*;
use lumys_types::capability::Capability;
use lumys_types::config::{KernelConfig, OutputFormat};
use lumys_types::error::LumysError;
use lumys_types::event::*;
use lumys_types::memory::Memory;
use lumys_types::tool::ToolDefinition;
...
/// The main Lumys OS kernel — coordinates all subsystems.
...
pub struct LumysKernel {
    ...
    pub model_catalog: std::sync::RwLock<lumys_runtime::model_catalog::ModelCatalog>,
    /// Skill registry for plugin skills (RwLock for hot-reload on install/uninstall).
    pub skill_registry: std::sync::RwLock<lumys_skills::registry::SkillRegistry>,
    ...
    pub mcp_connections: tokio::sync::Mutex<Vec<lumys_runtime::mcp::McpConnection>>,
    ...
    pub a2a_task_store: lumys_runtime::a2a::A2aTaskStore,
    /// Discovered external A2A agent cards.
    pub a2a_external_agents: std::sync::Mutex<Vec<(String, lumys_runtime::a2a::AgentCard)>>,
    /// Web tools context (multi-provider search + SSRF-protected fetch + caching).
    pub web_ctx: lumys_runtime::web_search::WebToolsContext,
    /// Browser automation manager (Playwright bridge sessions).
    pub browser_ctx: lumys_runtime::browser::BrowserManager,
    /// Media understanding engine (image description, audio transcription).
    pub media_engine: lumys_runtime::media_understanding::MediaEngine,
    /// Text-to-speech engine.
    pub tts_engine: lumys_runtime::tts::TtsEngine,
    ...
    pub embedding_driver:
        Option<Arc<dyn lumys_runtime::embedding::EmbeddingDriver + Send + Sync>>,
    /// Hand registry — curated autonomous capability packages.
    pub hand_registry: lumys_hands::registry::HandRegistry,
    /// Credential resolver — vault → dotenv → env var priority chain.
    pub credential_resolver: std::sync::Mutex<lumys_extensions::credentials::CredentialResolver>,
    /// Extension/integration registry (bundled MCP templates + install state).
    pub extension_registry: std::sync::RwLock<lumys_extensions::registry::IntegrationRegistry>,
    /// Integration health monitor.
    pub extension_health: lumys_extensions::health::HealthMonitor,
    /// Effective MCP server list (manual config + extension-installed, merged at boot).
    pub effective_mcp_servers: std::sync::RwLock<Vec<lumys_types::config::McpServerConfigEntry>>,
    ...
    pub bindings: std::sync::Mutex<Vec<lumys_types::config::AgentBinding>>,
    /// Broadcast configuration.
    pub broadcast: lumys_types::config::BroadcastConfig,
    /// Auto-reply engine.
    pub auto_reply_engine: crate::auto_reply::AutoReplyEngine,
    /// Plugin lifecycle hook registry.
    pub hooks: lumys_runtime::hooks::HookRegistry,
    /// Persistent process manager for interactive sessions (REPLs, servers).
    pub process_manager: Arc<lumys_runtime::process_manager::ProcessManager>,
    /// OFP peer registry — tracks connected peers (OnceLock for safe init after Arc creation).
    pub peer_registry: OnceLock<lumys_wire::PeerRegistry>,
    /// OFP peer node — the local networking node (OnceLock for safe init after Arc creation).
    pub peer_node: OnceLock<Arc<lumys_wire::PeerNode>>,
    ...
    pub channel_adapters:
        dashmap::DashMap<String, Arc<dyn lumys_channels::types::ChannelAdapter>>,
    /// Hot-reloadable default model override (set via config hot-reload, read at agent spawn).
    pub default_model_override:
        std::sync::RwLock<Option<lumys_types::config::DefaultModelConfig>>,
    ...
    self_handle: OnceLock<Weak<LumysKernel>>,
}

/// Bounded in-memory delivery receipt tracker.
/// Stores up to `MAX_RECEIPTS` most recent delivery receipts per agent.
pub struct DeliveryTracker {
    receipts: dashmap::DashMap<AgentId, Vec<lumys_channels::types::DeliveryReceipt>>,
}
...
impl DeliveryTracker {
    ...
    pub fn record(&self, agent_id: AgentId, receipt: lumys_channels::types::DeliveryReceipt) {
    ...
    pub fn get_receipts(
        ...
        agent_id: AgentId,
        limit: usize,
    ) -> Vec<lumys_channels::types::DeliveryReceipt> {
        self.receipts
            .get(&agent_id)
    ...
    pub fn sent_receipt(
        channel: &str,
        recipient: &str,
    ) -> lumys_channels::types::DeliveryReceipt {
        lumys_channels::types::DeliveryReceipt {
            message_id: uuid::Uuid::new_v4().to_string(),
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: lumys_channels::types::DeliveryStatus::Sent,
    ...
    pub fn failed_receipt(
        ...
        lumys_channels::types::DeliveryReceipt {
            ...
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: lumys_channels::types::DeliveryStatus::Failed,
            timestamp: chrono::Utc::now(),
...
fn ensure_workspace(workspace: &Path) -> KernelResult<()> {
    for subdir in &["data", "output", "sessions", "skills", "logs", "memory"] {
        std::fs::create_dir_all(workspace.join(subdir)).map_err(|e| {
            KernelError::Lumys OS(LumysError::Internal(format!(
...
fn append_daily_memory_log(workspace: &Path, response: &str) {
    ...
    let summary = lumys_types::truncate_str(trimmed, 500);
...
fn read_identity_file(workspace: &Path, filename: &str) -> Option<String> {
    ...
    if content.len() > MAX_IDENTITY_FILE_BYTES {
        Some(lumys_types::truncate_str(&content, MAX_IDENTITY_FILE_BYTES).to_string())
    } else {
        Some(content)
    }
}
...
impl LumysKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        match config.mode {
            KernelMode::Stable => {
                info!("Booting Lumys OS kernel in STABLE mode — conservative defaults enforced");
            }
            KernelMode::Dev => {
                warn!("Booting Lumys OS kernel in DEV mode — experimental features enabled");
            }
            KernelMode::Default => {
                info!("Booting Lumys OS kernel...");
            }
        }
        ...
        let db_path = config
            ...
            .sqlite_path
            .clone()
            .unwrap_or_else(|| config.data_dir.join("lumys.db"));
        let memory = Arc::new(
        ...
        let driver: Arc<dyn LlmDriver> = if driver_chain.len() > 1 {
            Arc::new(lumys_runtime::drivers::fallback::FallbackDriver::with_models(model_chain))
        ...
        let metering = Arc::new(MeteringEngine::new(Arc::new(
            lumys_memory::usage::UsageStore::new(memory.usage_conn()),
        )));
        ...
        let mut model_catalog = lumys_runtime::model_catalog::ModelCatalog::new();
        ...
        // Load user's custom models from ~/.lumys/custom_models.json
        ...
        let mut skill_registry = lumys_skills::registry::SkillRegistry::new(skills_dir);
        ...
        let hand_registry = lumys_hands::registry::HandRegistry::new();
        ...
        let mut extension_registry =
            lumys_extensions::registry::IntegrationRegistry::new(&config.home_dir);
        ...
        let health_config = lumys_extensions::health::HealthMonitorConfig {
        ...
        let extension_health = lumys_extensions::health::HealthMonitor::new(health_config);
        ...
        let web_cache = Arc::new(lumys_runtime::web_cache::WebCache::new(cache_ttl));
        let web_ctx = lumys_runtime::web_search::WebToolsContext {
            search: lumys_runtime::web_search::WebSearchEngine::new(
                config.web.clone(),
                web_cache.clone(),
            ),
            fetch: lumys_runtime::web_fetch::WebFetchEngine::new(
        ...
        let embedding_driver: Option<
            Arc<dyn lumys_runtime::embedding::EmbeddingDriver + Send + Sync>,
        > = {
            use lumys_runtime::embedding::create_embedding_driver;
        ...
        let browser_ctx = lumys_runtime::browser::BrowserManager::new(config.browser.clone());

        // Initialize media understanding engine
        let media_engine =
            lumys_runtime::media_understanding::MediaEngine::new(config.media.clone());
        let tts_engine = lumys_runtime::tts::TtsEngine::new(config.tts.clone());
        ...
        let kernel = Self {
            ...
            a2a_task_store: lumys_runtime::a2a::A2aTaskStore::default(),
            ...
            auto_reply_engine,
            hooks: lumys_runtime::hooks::HookRegistry::new(),
            process_manager: Arc::new(lumys_runtime::process_manager::ProcessManager::new(5)),
                ...
                for entry in agents {
                    ...
                    if toml_path.exists() {
                        match std::fs::read_to_string(&toml_path) {
                            Ok(toml_str) => {
                                match toml::from_str::<lumys_types::agent::AgentManifest>(
                                    &toml_str,
                                ) {
                                    Ok(disk_manifest) => {
        ...
        if kernel.registry.list().is_empty() {
            ...
            let manifest = AgentManifest {
                ...
                model: lumys_types::agent::ModelConfig {
        ...
        info!("Lumys OS kernel booted successfully");
        Ok(kernel)
    }
    ...
    pub fn spawn_agent_with_parent(
        ...
        self.audit_log.record(
            agent_id.to_string(),
            lumys_runtime::audit::AuditAction::AgentSpawn,
    ...
    pub fn verify_signed_manifest(&self, signed_json: &str) -> KernelResult<String> {
        let signed: lumys_types::manifest_signing::SignedManifest =
            serde_json::from_str(signed_json).map_err(|e| {
                KernelError::Lumys OS(lumys_types::error::LumysError::Config(format!(
    ...
    pub async fn send_message_with_blocks(
        ...
        agent_id: AgentId,
        message: &str,
        blocks: Vec<lumys_types::message::ContentBlock>,
    ) -> KernelResult<AgentLoopResult> {
        let handle: Option<Arc<dyn KernelHandle>> = self
            .self_handle
            .get()
    ...
    pub async fn send_message_with_handle_and_blocks(
        ...
        message: &str,
        kernel_handle: Option<Arc<dyn KernelHandle>>,
        content_blocks: Option<Vec<lumys_types::message::ContentBlock>>,
        sender_id: Option<String>,
        sender_name: Option<String>,
    ) -> KernelResult<AgentLoopResult> {
        ...
        let entry = self.registry.get(agent_id).ok_or_else(|| {
            KernelError::Lumys OS(LumysError::AgentNotFound(agent_id.to_string()))
        })?;
        ...
        match result {
            Ok(result) => {
                ...
                self.audit_log.record(
                    agent_id.to_string(),
                    lumys_runtime::audit::AuditAction::AgentMessage,
                    format!(
    ...
    pub fn send_message_streaming(
                    ...
                    Ok(result) => {
                        ...
                        let _ = tx
                            .send(StreamEvent::ContentComplete {
                                stop_reason: lumys_types::message::StopReason::EndTurn,
        ...
        let mut session = self
            ...
            .map_err(KernelError::Lumys OS)?
            .unwrap_or_else(|| lumys_memory::session::Session {
                id: entry.session_id,
                agent_id,
                messages: Vec::new(),
        ...
        // Build the structured system prompt via prompt_builder
        {
            ...
            let prompt_ctx = lumys_runtime::prompt_builder::PromptContext {
                ...
                workspace_context: manifest.workspace.as_ref().map(|w| {
                    let mut ws_ctx =
                        lumys_runtime::workspace_context::WorkspaceContext::detect(w);
            ...
            manifest.model.system_prompt =
                lumys_runtime::prompt_builder::build_system_prompt(&prompt_ctx);
            ...
            if let Some(cc_msg) =
                lumys_runtime::prompt_builder::build_canonical_context_message(&prompt_ctx)
            {
        ...
        let message_owned = if let Some(link_ctx) =
            lumys_runtime::link_understanding::build_link_context(message, &self.config.links)
        {
        ...
        let handle = tokio::spawn(async move {
            ...
            let phase_cb: lumys_runtime::agent_loop::PhaseCallback =
                std::sync::Arc::new(move |phase| {
                    use lumys_runtime::agent_loop::LoopPhase;
            ...
            match result {
                Ok(result) => {
                    ...
                    let _ = kernel_clone.metering.record(&lumys_memory::usage::UsageRecord {
                        agent_id,
                        model: model.clone(),
    ...
    async fn execute_wasm_agent(
        ...
        Ok(AgentLoopResult {
            response,
            total_usage: lumys_types::message::TokenUsage {
                input_tokens: 0,
                output_tokens: 0,
            },
            iterations: 1,
    ...
    async fn execute_llm_agent(
        ...
        let is_stable = self.config.mode == lumys_types::config::KernelMode::Stable;
        ...
        if is_stable {
            ...
            let probe = CompletionRequest {
                ...
                messages: vec![lumys_types::message::Message::user(message)],
                tools: tools.clone(),
        ...
        let _ = self.metering.record(&lumys_memory::usage::UsageRecord {
            agent_id,
            model: model.clone(),
            input_tokens: result.total_usage.input_tokens,
            output_tokens: result.total_usage.output_tokens,
        ...
        match self.config.usage_footer {
            lumys_types::config::UsageFooterMode::Off => {
                result.cost_usd = None;
            }
            lumys_types::config::UsageFooterMode::Cost
            | lumys_types::config::UsageFooterMode::Full => {
                result.cost_usd = if cost > 0.0 { Some(cost) } else { None };
            }
            lumys_types::config::UsageFooterMode::Tokens => {
    ...
    pub fn switch_agent_session(
        ...
        let session = self
            ...
            .map_err(KernelError::Lumys OS)?
            .ok_or_else(|| {
                KernelError::Lumys OS(LumysError::Internal("Session not found".to_string()))
            })?;

        if session.agent_id != agent_id {
            return Err(KernelError::Lumys OS(LumysError::Internal(
    ...
    fn save_session_summary(
        ...
        agent_id: AgentId,
        entry: &AgentEntry,
        session: &lumys_memory::session::Session,
    ) {
        use lumys_types::message::{MessageContent, Role};
        ...
        let summary = format!(
            ...
            topics
                ...
                .enumerate()
                .map(|(i, t)| {
                    let truncated = lumys_types::truncate_str(t, 200);
        ...
        if !skills.is_empty() {
            ...
            for name in &skills {
                if !known.contains(name) {
                    return Err(KernelError::Lumys OS(LumysError::Internal(format!(
                        "Unknown skill: {name}"
                    ))));
                }
            }
        ...
        if !servers.is_empty() {
            if let Ok(mcp_tools) = self.mcp_tools.lock() {
                ...
                for tool in mcp_tools.iter() {
                    if let Some(s) = lumys_runtime::mcp::extract_mcp_server(&tool.name) {
                        known_servers.insert(s.to_string());
                    }
                }
                for name in &servers {
                    let normalized = lumys_runtime::mcp::normalize_name(name);
        ...
        let (input_tokens, output_tokens) = session
            .map(|s| {
                ...
                for msg in &s.messages {
                    ...
                    let tokens = len / 4;
                    match msg.role {
                        lumys_types::message::Role::User => input += tokens,
                        lumys_types::message::Role::Assistant => output += tokens,
                        lumys_types::message::Role::System => input += tokens,
                    }
                }
                (input, output)
            })
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        use lumys_runtime::compactor::{compact_session, needs_compaction, CompactionConfig};
        ...
        let result = compact_session(driver, &model, &session, &config)
            .await
            .map_err(|e| KernelError::Lumys OS(LumysError::Internal(e)))?;
        ...
        let (repaired_messages, repair_stats) =
            lumys_runtime::session_repair::validate_and_repair_with_stats(&result.kept_messages);
    ...
    pub fn context_report(
        &self,
        agent_id: AgentId,
    ) -> KernelResult<lumys_runtime::compactor::ContextReport> {
        use lumys_runtime::compactor::generate_context_report;
    ...
    pub fn kill_agent(&self, agent_id: AgentId) -> KernelResult<()> {
        ...
        self.audit_log.record(
            agent_id.to_string(),
            lumys_runtime::audit::AuditAction::AgentKill,
    ...
    pub fn activate_hand(
    ...
    ) -> KernelResult<lumys_hands::HandInstance> {
        use lumys_hands::HandError;

        let def = self
            .hand_registry
            .get_definition(hand_id)
            .ok_or_else(|| {
                KernelError::Lumys OS(LumysError::AgentNotFound(format!(
        ...
        let instance = self
            ...
            .map_err(|e| match e {
                HandError::AlreadyActive(id) => KernelError::Lumys OS(LumysError::Internal(
                    format!("Hand already active: {id}"),
                )),
                other => KernelError::Lumys OS(LumysError::Internal(other.to_string())),
            })?;
        ...
        let mut manifest = AgentManifest {
            ...
            exec_policy: if def.tools.iter().any(|t| t == "shell_exec") {
                Some(lumys_types::config::ExecPolicy {
                    mode: lumys_types::config::ExecSecurityMode::Full,
        ...
        let resolved = lumys_hands::resolve_settings(&def.settings, &instance.config);
        ...
        for req in &def.requires {
            match req.requirement_type {
                lumys_hands::RequirementType::ApiKey
                | lumys_hands::RequirementType::EnvVar => {
        ...
        self.hand_registry
            ...
            .map_err(|e| KernelError::Lumys OS(LumysError::Internal(e.to_string())))?;
    ...
    pub fn list_bindings(&self) -> Vec<lumys_types::config::AgentBinding> {
        self.bindings
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    pub fn add_binding(&self, binding: lumys_types::config::AgentBinding) {
    ...
    pub fn remove_binding(&self, index: usize) -> Option<lumys_types::config::AgentBinding> {
        let mut bindings = self.bindings.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn apply_hot_actions(
        ...
        new_config: &lumys_types::config::KernelConfig,
    ) {
    ...
    pub fn register_trigger(
        ...
        if self.registry.get(agent_id).is_none() {
            return Err(KernelError::Lumys OS(LumysError::AgentNotFound(
                agent_id.to_string(),
            )));
        }
        Ok(self
    ...
    pub async fn run_workflow(
        ...
        let run_id = self
            ...
            .await
            .ok_or_else(|| {
                KernelError::Lumys OS(LumysError::Internal("Workflow not found".to_string()))
            })?;
        ...
        .map_err(|e| {
            KernelError::Lumys OS(LumysError::Internal(format!("Workflow failed: {e}")))
        })?;
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        let state_path = self.config.home_dir.join("hand_state.json");
        let saved_hands = lumys_hands::registry::HandRegistry::load_state(&state_path);
        ...
        let mut bg_agents: Vec<(lumys_types::agent::AgentId, String, ScheduleMode)> = Vec::new();
        ...
        // Probe local providers for reachability and model discovery
        {
            ...
            tokio::spawn(async move {
                ...
                for (provider_id, base_url) in &local_providers {
                    let result =
                        lumys_runtime::provider_health::probe_provider(provider_id, base_url)
                            .await;
                    if result.reachable {
                        info!(
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    for job in due {
                        ...
                        match &job.action {
                            lumys_types::scheduler::CronAction::SystemEvent { text } => {
                            ...
                            lumys_types::scheduler::CronAction::AgentTurn {
                                message,
                                timeout_secs,
                                ..
                            } => {
                                ...
                                let kh: std::sync::Arc<
                                    dyn lumys_runtime::kernel_handle::KernelHandle,
                                > = kernel.clone();
                            ...
                            lumys_types::scheduler::CronAction::WorkflowRun {
                                workflow_id,
                                input,
                                timeout_secs,
                            } => {
        ...
        if let Some(ref a2a_config) = self.config.a2a {
            if a2a_config.enabled && !a2a_config.external_agents.is_empty() {
                ...
                tokio::spawn(async move {
                    let discovered = lumys_runtime::a2a::discover_external_agents(&agents).await;
    ...
    async fn start_ofp_node(self: &Arc<Self>) {
        use lumys_wire::{PeerConfig, PeerNode, PeerRegistry};
        ...
        let node_name = gethostname().unwrap_or_else(|| "lumys-node".to_string());
        ...
        let handle: Arc<dyn lumys_wire::peer::PeerHandle> = self.self_arc();
    ...
    pub fn shutdown(&self) {
        info!("Shutting down Lumys OS kernel...");
        ...
        info!(
            "Lumys OS kernel shut down ({} agents preserved)",
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        if !manifest.fallback_models.is_empty() {
            ...
            let mut chain: Vec<(
                std::sync::Arc<dyn lumys_runtime::llm_driver::LlmDriver>,
                String,
            ...
            if chain.len() > 1 {
                return Ok(Arc::new(
                    lumys_runtime::drivers::fallback::FallbackDriver::with_models(chain),
                ));
            }
        }
    ...
    async fn connect_mcp_servers(self: &Arc<Self>) {
        use lumys_runtime::mcp::{McpConnection, McpServerConfig, McpTransport};
        use lumys_types::config::McpTransportEntry;
    ...
    fn available_tools(&self, agent_id: AgentId) -> Vec<ToolDefinition> {
        ...
        if let Ok(mcp_tools) = self.mcp_tools.lock() {
            let mcp_candidates: Vec<ToolDefinition> = if mcp_allowlist.is_empty() {
                ...
                let normalized: Vec<String> = mcp_allowlist
                    .iter()
                    .map(|s| lumys_runtime::mcp::normalize_name(s))
                    .collect();
                mcp_tools
                    .iter()
                    .filter(|t| {
                        lumys_runtime::mcp::extract_mcp_server(&t.name)
        ...
        let exec_blocks_shell = entry.as_ref().is_some_and(|e| {
            e.manifest
                .exec_policy
                .as_ref()
                .is_some_and(|p| p.mode == lumys_types::config::ExecSecurityMode::Deny)
        });
    ...
    pub fn reload_skills(&self) {
        ...
        let mut fresh = lumys_skills::registry::SkillRegistry::new(skills_dir);
...
fn apply_budget_defaults(
    budget: &lumys_types::config::BudgetConfig,
...
async fn cron_deliver_response(
    ...
    agent_id: AgentId,
    response: &str,
    delivery: &lumys_types::scheduler::CronDelivery,
) -> Result<(), String> {
    use lumys_types::scheduler::CronDelivery;
...
#[async_trait]
impl KernelHandle for LumysKernel {
    async fn spawn_agent(
        ...
        let content_hash = lumys_types::manifest_signing::hash_manifest(manifest_toml);
    ...
    fn kill_agent(&self, agent_id: &str) -> Result<(), String> {
        ...
        LumysKernel::kill_agent(self, id).map_err(|e| format!("Kill failed: {e}"))
    }
    ...
    async fn publish_event(
        ...
        LumysKernel::publish_event(self, event).await;
        Ok(())
    }

    async fn knowledge_add_entity(
        &self,
        entity: lumys_types::memory::Entity,
    ) -> Result<String, String> {
        self.memory
            .add_entity(entity)
            .await
    ...
    async fn knowledge_add_relation(
        &self,
        relation: lumys_types::memory::Relation,
    ) -> Result<String, String> {
        self.memory
    ...
    async fn knowledge_query(
        &self,
        pattern: lumys_types::memory::GraphPattern,
    ) -> Result<Vec<lumys_types::memory::GraphMatch>, String> {
        self.memory
            .query_graph(pattern)
            .await
    ...
    async fn cron_create(
        ...
        let aid = lumys_types::agent::AgentId(
    ...
    async fn cron_cancel(&self, job_id: &str) -> Result<(), String> {
        let id = lumys_types::scheduler::CronJobId(
    ...
    async fn request_approval(
        ...
        use lumys_types::approval::{ApprovalDecision, ApprovalRequest as TypedRequest};
    ...
    async fn send_channel_message(
        ...
        let user = lumys_channels::types::ChannelUser {
        ...
        let formatted = if channel == "wecom" {
            ...
            lumys_channels::formatter::format_for_wecom(message, output_format)
        } else {
            message.to_string()
        };

        let content = lumys_channels::types::ChannelContent::Text(formatted);
    ...
    async fn send_channel_media(
        ...
        let content = match media_type {
            "image" => lumys_channels::types::ChannelContent::Image {
                url: media_url.to_string(),
                caption: caption.map(|s| s.to_string()),
            },
            "file" => lumys_channels::types::ChannelContent::File {
                url: media_url.to_string(),
    ...
    async fn send_channel_file_data(
        ...
        let content = lumys_channels::types::ChannelContent::FileData {
            data,
    ...
    async fn spawn_agent_checked(
        ...
        manifest_toml: &str,
        parent_id: Option<&str>,
        parent_caps: &[lumys_types::capability::Capability],
    ) -> Result<(String, String), String> {
        ...
        lumys_types::capability::validate_capability_inheritance(parent_caps, &child_caps)?;
...
#[async_trait]
impl lumys_wire::peer::PeerHandle for LumysKernel {
    fn local_agents(&self) -> Vec<lumys_wire::message::RemoteAgentInfo> {
        self.registry
            .list()
            .iter()
            .map(|entry| lumys_wire::message::RemoteAgentInfo {
    ...
    fn discover_agents(&self, query: &str) -> Vec<lumys_wire::message::RemoteAgentInfo> {
        let q = query.to_lowercase();
        self.registry
            .list()
            .iter()
...
mod tests {
    ...
    fn test_hand_activation_does_not_seed_runtime_tool_filters() {
        ...
        let home_dir = tmp.path().join("lumys-kernel-hand-test");
        ...
        let kernel = LumysKernel::boot_with_config(config).expect("Kernel should boot");
        let instance = kernel
