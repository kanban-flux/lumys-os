# Lumys OS installer for Windows
# Usage: iwr -useb https://lumys.sh/install.ps1 | iex
#   or:  powershell -c "irm https://lumys.sh/install.ps1 | iex"
#
# Flags (via environment variables):
#   $env:LUMYS_INSTALL_DIR = custom install directory
#   $env:LUMYS_VERSION     = specific version tag (e.g. "v0.1.0")

$ErrorActionPreference = 'Stop'

$Repo = "ENI-Ethereal-Nexus-Institute/lumys-os"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".lumys\bin"
$InstallDir = if ($env:LUMYS_INSTALL_DIR) { $env:LUMYS_INSTALL_DIR } else { $DefaultInstallDir }

function Write-Banner {
    Write-Host ""
    Write-Host "  Lumys OS Installer" -ForegroundColor Cyan
...
function Get-Architecture {
    ...
    switch ($archUpper) {
        ...
        default {
            ...
            Write-Host "  Try: cargo install --git https://github.com/ENI-Ethereal-Nexus-Institute/lumys-os lumys-cli" -ForegroundColor Yellow
            exit 1
        }
    }
}
...
function Get-LatestVersion {
    if ($env:LUMYS_VERSION) {
        return $env:LUMYS_VERSION
    }
    ...
    try {
        ...
        Write-Host "    cargo install --git https://github.com/$Repo lumys-cli"
        exit 1
    }
}

function Install-Lumys {
    Write-Banner

    $arch = Get-Architecture
    $version = Get-LatestVersion
    $target = "${arch}-pc-windows-msvc"
    $archive = "lumys-${target}.zip"
    $url = "https://github.com/$Repo/releases/download/$version/$archive"
    $checksumUrl = "$url.sha256"

    Write-Host "  Installing Lumys OS $version for $target..."
    ...
    # Download to temp
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "lumys-install"
    ...
    $exePath = Join-Path $tempDir "lumys.exe"
    if (-not (Test-Path $exePath)) {
        # May be nested in a directory
        $found = Get-ChildItem -Path $tempDir -Filter "lumys.exe" -Recurse | Select-Object -First 1
        if ($found) {
            $exePath = $found.FullName
        }
        else {
            Write-Host "  Could not find lumys.exe in archive." -ForegroundColor Red
            Remove-Item -Recurse -Force $tempDir -ErrorAction SilentlyContinue
            exit 1
        }
    }
    ...
    # Install
    Copy-Item -Path $exePath -Destination (Join-Path $InstallDir "lumys.exe") -Force
    ...
    # Verify
    $installedExe = Join-Path $InstallDir "lumys.exe"
    if (Test-Path $installedExe) {
        try {
            $versionOutput = & $installedExe --version 2>&1
            Write-Host ""
            Write-Host "  Lumys OS installed successfully! ($versionOutput)" -ForegroundColor Green
        }
        catch {
            Write-Host ""
            Write-Host "  Lumys OS binary installed to $installedExe" -ForegroundColor Green
        }
    }

    Write-Host ""
    Write-Host "  Get started:" -ForegroundColor Cyan
    Write-Host "    lumys init"
    Write-Host ""
