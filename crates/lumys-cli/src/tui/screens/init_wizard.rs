...
//!
//! Launched by "`lumys init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use lumys_runtime::model_catalog::ModelCatalog;
use lumys_types::model_catalog::ModelTier;
...
struct State {
    ...
    openclaw_scan: Option<lumys_migrate::openclaw::ScanResult>,
    migration_report: Option<lumys_migrate::report::MigrationReport>,
...
impl State {
    ...
    fn build_provider_order(&mut self) {
        ...
        for (i, p) in PROVIDERS.iter().enumerate() {
            let detected = if p.name == "claude-code" {
                lumys_runtime::drivers::claude_code::claude_code_available()
            } else {
    ...
    fn is_provider_detected(&self, prov_idx: usize) -> bool {
        ...
        if p.name == "claude-code" {
            return lumys_runtime::drivers::claude_code::claude_code_available();
        }
        (!p.env_var.is_empty() && std::env::var(p.env_var).is_ok())
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<lumys_migrate::report::MigrationReport, String>>();
...
fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    migrate_tx: &std::sync::mpsc::Sender<Result<lumys_migrate::report::MigrationReport, String>>,
) {
        ...
        MigrationPhase::Offer => match code {
            ...
            KeyCode::Enter => {
                ...
                if yes {
                    ...
                    let target_dir = if let Ok(h) = std::env::var("LUMYS_HOME") {
                        PathBuf::from(h)
                    } else {
                        dirs::home_dir()
                    ...
                    std::thread::spawn(move || {
                        let options = lumys_migrate::MigrateOptions {
                            source: lumys_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            lumys_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let lumys_dir = if let Ok(h) = std::env::var("LUMYS_HOME") {
        PathBuf::from(h)
    } else {
        match dirs::home_dir() {
            Some(h) => h.join(".lumys"),
    ...
    let _ = std::fs::create_dir_all(lumys_dir.join("agents"));
    let _ = std::fs::create_dir_all(lumys_dir.join("data"));
    crate::restrict_dir_permissions(&lumys_dir);
...
/// Check if the `lumys-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "Lumys OS Init  Step X of 7"
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use lumys_migrate::report::ItemKind;
...
fn draw_routing_pick(f: &mut Frame, area: Rect, state: &mut State, tier: usize) {
        ...
        .enumerate()
    {
        if t == tier {
            ...
            let display = lumys_types::truncate_str(short, 14);
            summary_spans.push(Span::styled(
                format!("{name}:{display}"),
                Style::default().fg(*c),
            ));
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use Lumys OS?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
