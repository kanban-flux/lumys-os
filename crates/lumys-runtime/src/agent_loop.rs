...
use lumys_memory::session::Session;
use lumys_memory::MemorySubstrate;
use lumys_skills::registry::SkillRegistry;
use lumys_types::agent::{AgentManifest, FallbackModel};
use lumys_types::error::{LumysError, LumysResult};
use lumys_types::memory::{Memory, MemoryFilter, MemorySource};
use lumys_types::message::{
    ContentBlock, Message, MessageContent, Role, StopReason, TokenUsage,
};
use lumys_types::tool::{ToolCall, ToolDefinition};
...
pub struct AgentLoopResult {
    ...
    pub directives: lumys_types::message::ReplyDirectives,
}

/// Run the agent execution loop for a single user message.
///
/// This is the core of Lumys OS: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: lumys_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if text.trim() == "NO_REPLY" || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: lumys_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: lumys_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in &response.tool_calls {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(LumysError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: lumys_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: lumys_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(LumysError::MaxIterationsExceeded(max_iterations))
}
...
) -> LumysResult<crate::llm_driver::CompletionResponse> {
    ...
    if let (Some(provider), Some(cooldown)) = (provider, cooldown) {
        match cooldown.check(provider) {
            CooldownVerdict::Reject {
                ...
                return Err(LumysError::LlmDriver(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(LumysError::LlmDriver(user_msg));
            }
        }
    }

    Err(LumysError::LlmDriver(
...
mod tests {
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = lumys_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = lumys_types::agent::AgentId::new();
        let mut session = lumys_memory::session::Session {
            id: lumys_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),
