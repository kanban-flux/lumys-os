...
use lumys_skills::registry::SkillRegistry;
use lumys_types::taint::{TaintLabel, TaintSink, TaintedValue};
use lumys_types::tool::{ToolDefinition, ToolResult};
use lumys_types::tool_compat::normalize_tool_name;
...
pub async fn execute_tool(
    ...
    exec_policy: Option<&lumys_types::config::ExecPolicy>,
    tts_engine: Option<&crate::tts::TtsEngine>,
    docker_config: Option<&lumys_types::config::DockerSandboxConfig>,
    process_manager: Option<&crate::process_manager::ProcessManager>,
) -> ToolResult {
    // Normalize the tool name through compat mappings so LLM-hallucinated aliases
    // (e.g. "fs-write" → "file_write") resolve to the canonical Lumys OS name.
    ...
    if let Some(kh) = kernel {
        if kh.requires_approval(tool_name) {
            ...
            let summary = format!(
                "{}: {}",
                tool_name,
                lumys_types::truncate_str(&input_str, 200)
            );
    ...
    let result = match tool_name {
        ...
        "shell_exec" => {
            ...
            let is_full_exec = exec_policy
                .is_some_and(|p| p.mode == lumys_types::config::ExecSecurityMode::Full);
            if !is_full_exec {
        ...
        other => {
            ...
            // Fallback 2: Skill registry tool providers
                ...
                if let Some(skill) = registry.find_tool_provider(other) {
                    ...
                    match lumys_skills::loader::execute_skill_tool(
                        &skill.manifest,
                        &skill.path,
                        other,
                        input,
    ...
    vec![
        ...
        ToolDefinition {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Base URL of the remote Lumys OS/A2A-compatible agent (e.g., 'https://agent.example.com')" }
                },
                "required": ["url"]
            }),
        },
...
async fn tool_web_search_legacy(input: &serde_json::Value) -> Result<String, String> {
    ...
    let resp = client
        ...
        .query(&[("q", query)])
        .header("User-Agent", "Mozilla/5.0 (compatible; LumysAgent/0.1)")
        .send()
        .await
...
async fn tool_shell_exec(
    ...
    let use_direct_exec = exec_policy
        .map(|p| p.mode == lumys_types::config::ExecSecurityMode::Allowlist)
...
fn parse_entity_type(s: &str) -> lumys_types::memory::EntityType {
    use lumys_types::memory::EntityType;
...
fn parse_relation_type(s: &str) -> lumys_types::memory::RelationType {
    use lumys_types::memory::RelationType;
    match s.to_lowercase().as_str() {
...
async fn tool_knowledge_add_entity(
    ...
    let entity = lumys_types::memory::Entity {
...
async fn tool_knowledge_add_relation(
    ...
    let relation = lumys_types::memory::Relation {
...
async fn tool_knowledge_query(
    ...
    let pattern = lumys_types::memory::GraphPattern {
        source,
        relation,
        target,
        max_depth,
...
async fn tool_location_get() -> Result<String, String> {
    ...
    let resp = client
        ...
        .header("User-Agent", "Lumys OS/0.1")
        .send()
        .await
...
async fn tool_media_describe(
    ...
    let attachment = lumys_types::media::MediaAttachment {
        media_type: lumys_types::media::MediaType::Image,
        mime_type: mime.to_string(),
        source: lumys_types::media::MediaSource::Base64 {
...
async fn tool_image_generate(
    ...
    let model = match model_str {
        "dall-e-3" | "dalle3" | "dalle-3" => lumys_types::media::ImageGenModel::DallE3,
        "dall-e-2" | "dalle2" | "dalle-2" => lumys_types::media::ImageGenModel::DallE2,
        "gpt-image-1" | "gpt_image_1" => lumys_types::media::ImageGenModel::GptImage1,
        _ => {
            return Err(format!(
    ...
    let request = lumys_types::media::ImageGenRequest {
...
async fn tool_speech_to_text(
    ...
    use lumys_types::media::{MediaAttachment, MediaSource, MediaType};
...
mod tests {
    ...
    async fn test_file_read_missing() {
        let bad_path = std::env::temp_dir()
            .join("lumys_test_nonexistent_99999")
            .join("file.txt");
    ...
    async fn test_capability_enforcement_allowed() {
        ...
        let bad_path = std::env::temp_dir()
            .join("lumys_test_nonexistent_12345")
            .join("file.txt");
        let result = execute_tool(
            "test-id",
            "file_read",
    ...
    async fn test_canvas_present_tool() {
        ...
        let tmp = std::env::temp_dir().join("lumys_canvas_test");
