...
//! to Lumys OS users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use lumys_types::agent::UserId;
use lumys_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their Lumys OS user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Lumys OS UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> LumysResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| LumysError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(LumysError::AuthDenied(format!(
