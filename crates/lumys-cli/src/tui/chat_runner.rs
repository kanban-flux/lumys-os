//! Standalone chat TUI for `lumys chat`.
//!
...
enum Backend {
    ...
    InProcess { kernel: Arc<LumysKernel> },
    None,
}
...
impl StandaloneChat {
    ...
    fn handle_stream_done(
        &mut self,
        result: Result<lumys_runtime::agent_loop::AgentLoopResult, String>,
    ) {
    ...
    fn handle_kernel_ready(&mut self, kernel: Arc<LumysKernel>) {
        self.booting = false;
    ...
    fn resolve_daemon_agent(&mut self, base_url: &str, agent_name: Option<&str>) {
        ...
        match template {
            ...
            None => {
                self.boot_error =
                    Some("No agent templates found. Run "`lumys init`.".to_string());
            }
        }
    }
