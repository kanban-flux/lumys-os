# Lumys OS Security Architecture

> **Security Contact:** jaber@lumys.sh — Report vulnerabilities via email. We respond within 48 hours.

This document provides a comprehensive technical reference for every security
system in the Lumys OS Agent Operating System.  All struct names, function
...
Lumys OS implements **defense-in-depth** security.  No single mechanism is
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `lumys-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `lumys-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `lumys-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `lumys-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `lumys-types` | Supply chain attacks |
| 6 | SSRF Protection | `lumys-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `lumys-runtime`, `lumys-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `lumys-wire` | Unauthorized peer connections |
| 9 | Security Headers | `lumys-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `lumys-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `lumys-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `lumys-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `lumys-skills` | Malicious skill prompts |
| 14 | Loop Guard | `lumys-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `lumys-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `lumys-api` | Information leakage |

---

## 2. Capability-Based Security

**Source:** `lumys-types/src/capability.rs`

Lumys OS uses capability-based security.  An agent can only perform actions
...
**Source:** `lumys-runtime/src/sandbox.rs`
...
**Source:** `lumys-runtime/src/audit.rs`
...
**Source:** `lumys-types/src/taint.rs`

Lumys OS implements a lattice-based taint propagation model that prevents
...
**Source:** `lumys-types/src/manifest_signing.rs`
...
**Source:** `lumys-runtime/src/host_functions.rs`
...
Lumys OS uses `Zeroizing<String>` from the `zeroize` crate on every field
...
**LLM Drivers** (`lumys-runtime/src/drivers/`):
...
**Channel Adapters** (`lumys-channels/src/`):
...
**Web Search** (`lumys-runtime/src/web_search.rs`):
...
**Embedding** (`lumys-runtime/src/embedding.rs`):
...
**Source:** `lumys-wire/src/peer.rs`

The Lumys OS Wire Protocol (OFP) uses HMAC-SHA256 with nonce-based mutual
...
**Source:** `lumys-api/src/middleware.rs`
...
**Source:** `lumys-api/src/rate_limiter.rs`

Lumys OS uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
**Source:** `lumys-runtime/src/subprocess_sandbox.rs`
...
**Source:** `lumys-skills/src/verify.rs`
...
**Source:** `lumys-runtime/src/loop_guard.rs`
...
**Source:** `lumys-runtime/src/session_repair.rs`
...
**Source:** `lumys-api/src/routes.rs`

Lumys OS provides two health endpoints with different information levels.
