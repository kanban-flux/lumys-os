...
//!
//! Allows any OpenAI-compatible client library to talk to Lumys OS agents.
//! The `model` field resolves to an agent (by name, UUID, or `lumys:<name>`),
...
use lumys_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "lumys:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("lumys:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("lumys:{}", e.name),
            object: "model",
            created,
            owned_by: "lumys".to_string(),
        })
        .collect();
