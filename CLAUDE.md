# Lumys OS — Agent Instructions

## Project Overview
Lumys OS is an open-source Agent Operating System written in Rust (14 crates).
- Config: `~/.lumys/config.toml`
- Default API: `http://127.0.0.1:4200`
- CLI binary: `target/release/lumys.exe` (or `target/debug/lumys.exe`)
...
```bash
GROQ_API_KEY=<key> target/release/lumys.exe start &
...
## Architecture Notes
- **Don't touch `lumys-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `lumys.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
