...
use chrono::Utc;
use lumys_types::agent::{AgentId, SessionId};
use lumys_types::error::{LumysError, LumysResult};
use lumys_types::message::{ContentBlock, Message, MessageContent, Role};
...
impl SessionStore {
        ...
        match result {
            Ok((agent_str, messages_blob, tokens, label)) => {
                ...
                let messages: Vec<Message> = rmp_serde::from_slice(&messages_blob)
                    .map_err(|e| LumysError::Serialization(e.to_string()))?;
                Ok(Some(Session {
                    id: session_id,
                    agent_id,
                    messages,
    ...
    pub fn list_sessions(&self) -> LumysResult<Vec<serde_json::Value>> {
        ...
        for row in rows {
            sessions.push(row.map_err(|e| LumysError::Memory(e.to_string()))?);
        }
        Ok(sessions)
    }
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> LumysResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
            ...
            if to_compact > canonical.compaction_cursor {
                ...
                for msg in compacting {
                    let role = match msg.role {
                        lumys_types::message::Role::User => "User",
                        lumys_types::message::Role::Assistant => "Assistant",
                        lumys_types::message::Role::System => "System",
                    };
                    let text = msg.content.text_content();
                    if !text.is_empty() {
                        // Truncate individual messages in summary to keep it compact (UTF-8 safe)
                        let truncated = if text.len() > 200 {
                            format!("{}...", lumys_types::truncate_str(&text, 200))
                        } else {
                            text
                        };
    ...
    ) -> LumysResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> LumysResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
                        ...
                        match block {
                            ...
                            ContentBlock::Thinking { thinking } => {
                                text_parts.push(format!(
                                    "[thinking: {}]",
                                    lumys_types::truncate_str(thinking, 200)
                                ));
                            }
...
mod tests {
    ...
    fn test_jsonl_mirror_write() {
        ...
        session
            .messages
            .push(lumys_types::message::Message::user("Hello"));
        session
            .messages
            .push(lumys_types::message::Message::assistant("Hi there!"));
        store.save_session(&session).unwrap();
