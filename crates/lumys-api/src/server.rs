//! Lumys OS daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.lumys/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. lumys-desktop)
...
pub async fn build_router(
    ...
    let state = Arc::new(AppState {
        ...
        provider_probe_cache: lumys_runtime::provider_health::ProbeCache::new(),
    });
...
/// Start the Lumys OS daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    info!("Lumys OS API server listening on http://{addr}");
...
/// Check if an Lumys OS daemon is actually responding at the given address.
