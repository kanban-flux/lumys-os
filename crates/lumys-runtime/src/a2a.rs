...
//! - `build_agent_card` — expose Lumys OS agents via A2A
//! - `A2aClient` — discover and interact with external A2A agents

use lumys_types::agent::AgentManifest;
...
pub struct AgentCard {
    ...
    /// Skills this agent can perform (A2A skill descriptors, not Lumys OS skills).
...
/// A2A skill descriptor (not an Lumys OS skill — describes a capability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn discover_external_agents(
    agents: &[lumys_types::config::ExternalAgent],
...
// ---------------------------------------------------------------------------
// A2A Server — expose Lumys OS agents via A2A
// ---------------------------------------------------------------------------

/// Build an A2A Agent Card from an Lumys OS agent manifest.
...
impl A2aClient {
    ...
    pub async fn discover(&self, url: &str) -> Result<AgentCard, String> {
        ...
        let response = self
            .client
            .get(&agent_json_url)
            .header("User-Agent", "Lumys OS/0.1 A2A")
            .send()
            .await
...
mod tests {
    ...
    fn test_a2a_config_serde() {
        use lumys_types::config::{A2aConfig, ExternalAgent};
