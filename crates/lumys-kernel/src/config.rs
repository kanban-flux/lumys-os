//! Configuration loading from `~/.lumys/config.toml` with defaults.
//!
...
use lumys_types::config::KernelConfig;
...
///
/// Respects `LUMYS_HOME` env var (e.g. `LUMYS_HOME=/opt/lumys`).
pub fn default_config_path() -> PathBuf {
    lumys_home().join("config.toml")
}

/// Get the Lumys OS home directory.
///
/// Priority: `LUMYS_HOME` env var > `~/.lumys`.
pub fn lumys_home() -> PathBuf {
