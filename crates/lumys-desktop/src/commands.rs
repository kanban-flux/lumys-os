...
use lumys_kernel::config::lumys_home;
...
/// `~/.lumys/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = lumys_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.lumys/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = lumys_home().join("skills");
...
/// Open the Lumys OS config directory (`~/.lumys/`) in the OS file manager.
#[tauri::command]
...
/// Open the Lumys OS logs directory (`~/.lumys/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = lumys_home().join("logs");
