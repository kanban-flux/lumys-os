// Lumys OS Setup Wizard — First-run guided setup (Provider + Agent + Channel)
'use strict';
...
function wizardPage() {
  return {
    ...
    async sendTryItMessage(text) {
      ...
      try {
        var res = await Lumys OSAPI.post('/api/agents/' + this.createdAgent.id + '/message', { message: text });
    ...
    async saveKey() {
      ...
      if (!key) {
        Lumys OSToast.error('Please enter an API key');
        return;
      }
      this.savingKey = true;
      try {
        await Lumys OSAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key });
        this.apiKeyInput = '';
        this.keySaved = true;
    ...
    async detectClaudeCode() {
      ...
      this.testResult = null;
      try {
        var result = await Lumys OSAPI.post('/api/providers/claude-code/test', {});
        this.testResult = result;
        if (result.status === 'ok') {
          this.claudeCodeDetected = true;
          this.keySaved = true;
          this.setupSummary.provider = 'Claude Code';
          Lumys OSToast.success('Claude Code detected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          this.testResult = { status: 'error', error: 'Claude Code CLI not detected' };
          Lumys OSToast.error('Claude Code CLI not detected. Make sure you\'ve run: npm install -g @anthropic-ai/claude-code && claude auth');
        }
      } catch(e) {
        this.testResult = { status: 'error', error: e.message };
        Lumys OSToast.error('Claude Code CLI not detected. Make sure you\'ve run: npm install -g @anthropic-ai/claude-code && claude auth');
    ...
    async createAgent() {
      ...
      if (!name) {
        Lumys OSToast.error('Please enter a name for your agent');
        return;
      }
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          Lumys OSToast.success('Agent "' + (res.name || name) + '" created');
          await Alpine.store('app').refreshAgents();
        } else {
          Lumys OSToast.error('Failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        Lumys OSToast.error('Failed to create agent: ' + e.message);
      }
    ...
    async configureChannel() {
      ...
      if (!token) {
        Lumys OSToast.error('Please enter the ' + ch.token_label);
        return;
      }
      ...
      try {
        ...
        fields.token = token;
        await Lumys OSAPI.post('/api/channels/' + ch.name + '/configure', { fields: fields });
        this.channelConfigured = true;
        this.setupSummary.channel = ch.display_name;
        Lumys OSToast.success(ch.display_name + ' configured and activated.');
      } catch(e) {
        Lumys OSToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
