...
cargo tauri signer generate -w ~/.tauri/lumys.key
```
...
Your private key was saved to: ~/.tauri/lumys.key
```
...
Open `crates/lumys-desktop/tauri.conf.json` and replace:
...
|---|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/lumys.key` | Yes |
...
The following icon files must exist in `crates/lumys-desktop/icons/`:
...
## 5. Set Up the `lumys.sh` Domain

**Status:** BLOCKING for install scripts — users run `curl -sSf https://lumys.sh | sh`.

Options:
- **GitHub Pages**: Point `lumys.sh` to a GitHub Pages site that redirects `/` to `scripts/install.sh` and `/install.ps1` to `scripts/install.ps1` from the repo's latest release.
- **Cloudflare Workers / Vercel**: Serve the install scripts with proper `Content-Type: text/plain` headers.
- **Raw GitHub redirect**: Use `lumys.sh` as a CNAME to `raw.githubusercontent.com/ENI-Ethereal-Nexus-Institute/lumys-os/main/scripts/install.sh` (less reliable).

The install scripts reference:
- `https://lumys.sh` → serves `scripts/install.sh`
- `https://lumys.sh/install.ps1` → serves `scripts/install.ps1`

Until the domain is set up, users can install via:
```bash
curl -sSf https://raw.githubusercontent.com/ENI-Ethereal-Nexus-Institute/lumys-os/main/scripts/install.sh | sh
```
...
```bash
docker build -t lumys:local .
docker run --rm lumys:local --version
docker run --rm -p 4200:4200 -v lumys-data:/data lumys:local start
```
...
grep '"version"' crates/lumys-desktop/tauri.conf.json
...
Visit: `https://github.com/ENI-Ethereal-Nexus-Institute/lumys-os/releases/latest/download/latest.json`
...
docker pull ghcr.io/ENI-Ethereal-Nexus-Institute/lumys-os:0.1.0

# Verify both architectures
docker run --rm ghcr.io/ENI-Ethereal-Nexus-Institute/lumys-os:latest --version
```
...
4. Open the v0.1.0 app — after 10 seconds it should:
   - Show "Lumys OS Updating..." notification
...
lumys --version  # Should print v0.1.0
