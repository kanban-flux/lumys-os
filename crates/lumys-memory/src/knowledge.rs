...
impl KnowledgeStore {
    ...
    pub fn add_entity(&self, entity: Entity) -> LumysResult<String> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn add_relation(&self, relation: Relation) -> LumysResult<String> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_graph(&self, pattern: GraphPattern) -> LumysResult<Vec<GraphMatch>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
        ...
        for row_result in rows {
            let r = row_result.map_err(|e| LumysError::Memory(e.to_string()))?;
