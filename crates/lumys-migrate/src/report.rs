...
impl MigrationReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str("1. Review imported agent manifests in `~/.lumys/agents/`\n");
        out.push_str(
            "2. Review `~/.lumys/secrets.env` — verify tokens were migrated correctly\n",
        );
        out.push_str("3. Set any remaining API keys referenced in `~/.lumys/config.toml`\n");
        out.push_str("4. Start the daemon: "`lumys start`\n");
        out.push_str("5. Test your agents: "`lumys agent list`\n");
    ...
    fn test_report_with_items() {
        let report = MigrationReport {
            ...
            imported: vec![MigrateItem {
                ...
                destination: "~/.lumys/agents/coder/agent.toml".to_string(),
            }],
