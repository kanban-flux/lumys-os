/**
 * @lumys/sdk — Official JavaScript client for the Lumys OS Agent OS REST API.
 *
 * Usage:
 *   const { Lumys OS } = require("@lumys/sdk");
 *   const client = new Lumys OS("http://localhost:3000");
 *
...
class Lumys OSError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "Lumys OSError";
    this.status = status;
    this.body = body;
  }
}
...
class Lumys OS {
  /**
   * @param {string} baseUrl - Lumys OS server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new Lumys OSError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new Lumys OSError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { Lumys OS: Lumys OS, Lumys OSError: Lumys OSError };
