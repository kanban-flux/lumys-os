...
async fn handle_text_message(
    ...
    match msg_type {
        "message" => {
            ...
            let mut ws_content_blocks: Option<Vec<lumys_types::message::ContentBlock>> = None;
            ...
            match state.kernel.send_message_streaming(
                ...
                Ok((mut rx, handle)) => {
                    ...
                    let stream_task = tokio::spawn(async move {
                        ...
                        let mut stream_usage: Option<lumys_types::message::TokenUsage> = None;
...
async fn handle_command(
    ...
    match cmd {
        ...
        "context" => match state.kernel.context_report(agent_id) {
            Ok(report) => {
                let formatted = lumys_runtime::compactor::format_context_report(&report);
                serde_json::json!({
                    "type": "command_result",
                    "command": cmd,
                    "message": formatted,
...
///
/// Uses the proper LLM error classifier from `lumys_runtime::llm_errors`
/// for comprehensive 20-provider coverage with actionable advice.
fn classify_streaming_error(err: &lumys_kernel::error::KernelError) -> String {
