//! Minimal `.env` file loader/saver for `~/.lumys/.env`.
//!
...
fn dotenv_lumys_home() -> Option<PathBuf> {
    if let Ok(home) = std::env::var("LUMYS_HOME") {
        return Some(PathBuf::from(home));
    }
    dirs::home_dir().map(|h| h.join(".lumys"))
}

/// Return the path to `~/.lumys/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dotenv_lumys_home().map(|h| h.join(".env"))
}

/// Load `~/.lumys/.env` and `~/.lumys/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.lumys/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dotenv_lumys_home().map(|h| h.join("secrets.env"))
}
...
/// List key names (without values) from `~/.lumys/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# Lumys OS environment — managed by "`lumys config set-key`\n");
