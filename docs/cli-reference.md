# Lumys OS CLI Reference

Complete command-line reference for `lumys`, the CLI tool for the Lumys OS Agent OS.

## Overview

The `lumys` binary is the primary interface for managing the Lumys OS Agent OS. It supports two modes of operation:

- **Daemon mode** -- When a daemon is running (`lumys start`), CLI commands communicate with it over HTTP. This is the recommended mode for production use.
...
Running `lumys` with no subcommand launches the interactive TUI (terminal user interface) built with ratatui, which provides a full dashboard experience in the terminal.
...
```bash
cargo build --release -p lumys-cli
# Binary: target/release/lumys (or lumys.exe on Windows)
```

### Docker

```bash
docker run -it lumys/lumys:latest
```

### Shell installer

```bash
curl -fsSL https://get.lumys.ai | sh
```
...
| Option | Description |
|---|---|
| `--config <PATH>` | Path to a custom config file. Overrides the default `~/.lumys/config.toml`. |
| `--help` | Print help information for any command or subcommand. |
| `--version` | Print the version of the `lumys` binary. |

**Environment variables:**

| Variable | Description |
|---|---|
| `RUST_LOG` | Controls log verbosity (e.g. `info`, `debug`, `lumys_kernel=trace`). |
| `LUMYS_AGENTS_DIR` | Override the agent templates directory. |
| `EDITOR` / `VISUAL` | Editor used by `lumys config edit`. Falls back to `notepad` (Windows) or `vi` (Unix). |
...
The TUI provides a full-screen terminal interface with panels for agents, chat, workflows, channels, skills, settings, and more. Tracing output is redirected to `~/.lumys/tui.log` to avoid corrupting the terminal display.
...
Initialize the Lumys OS workspace. Creates `~/.lumys/` with subdirectories (`data/`, `agents/`) and a default `config.toml`.
...
Start the Lumys OS daemon (kernel + API server).

```
lumys start [--config <PATH>]
```

**Behavior:**

- Checks if a daemon is already running; exits with an error if so.
- Boots the Lumys OS kernel (loads config, initializes SQLite database, loads agents, connects MCP servers, starts background tasks).
- Starts the HTTP API server on the address specified in `config.toml` (default: `127.0.0.1:4200`).
- Writes `daemon.json` to `~/.lumys/` so other CLI commands can discover the running daemon.
- Blocks until interrupted with `Ctrl+C`.

**Output:**

```
  Lumys OS Agent OS v0.1.0
  ...
  hint: Open the dashboard in your browser, or run `lumys chat`
...
lumys start --config /path/to/config.toml
```
...
lumys status --json | jq '.agent_count'
```
...
Run diagnostic checks on the Lumys OS installation.

```
lumys doctor [--json] [--repair]
```
...
1. **Lumys OS directory** -- `~/.lumys/` exists
...
9. **Agent manifests** -- validates all `.toml` files in `~/.lumys/agents/`
...
```bash
# Bash
lumys completion bash > ~/.bash_completion.d/lumys

# Zsh
lumys completion zsh > ~/.zfunc/_lumys

# Fish
lumys completion fish > ~/.config/fish/completions/lumys.fish

# PowerShell
lumys completion powershell > lumys.ps1
```
...
- Templates are discovered from: the repo `agents/` directory (dev builds), `~/.lumys/agents/` (installed), and `LUMYS_AGENTS_DIR` (env override).
...
# Spawn by name
lumys agent new coder

# Spawn the assistant template
lumys agent new assistant
```
...
```bash
lumys agent spawn ./my-agent/agent.toml
```
...
| Argument | Description |
|---|---|
| `<AGENT_ID>` | Agent UUID. Obtain from `lumys agent list`. |
...
```bash
lumys agent chat a1b2c3d4-e5f6-7890-abcd-ef1234567890
```
...
```bash
lumys agent kill a1b2c3d4-e5f6-7890-abcd-ef1234567890
```
...
### lumys workflow create

Create a workflow from a JSON definition file.

```
lumys workflow create <FILE>
```
...
```bash
lumys workflow create ./my-workflow.json
```
...
```
lumys workflow run <WORKFLOW_ID> <INPUT>
```
...
| Argument | Description |
|---|---|
| `<WORKFLOW_ID>` | Workflow UUID. Obtain from `lumys workflow list`. |
| `<INPUT>` | Input text to pass to the workflow. |

**Example:**

```bash
lumys workflow run abc123 "Analyze this code for security issues"
```
...
```
lumys trigger list [--agent-id <ID>]
```
...
### lumys trigger create

Create an event trigger for an agent.

```
lumys trigger create <AGENT_ID> <PATTERN_JSON> [--prompt <TEMPLATE>] [--max-fires <N>]
```
...
lumys trigger create <AGENT_ID> '{"lifecycle":{}}'

# Fire when a specific agent is spawned
lumys trigger create <AGENT_ID> '{"agent_spawned":{"name_pattern":"*"}}'

# Fire on agent termination
lumys trigger create <AGENT_ID> '{"agent_terminated":{}}'

# Fire on all events (limited to 10 fires)
lumys trigger create <AGENT_ID> '{"all":{}}' --max-fires 10
```

---

### lumys trigger delete

Delete a trigger by ID.

```
lumys trigger delete <TRIGGER_ID>
```
...
Loads skills from `~/.lumys/skills/` plus bundled skills compiled into the binary.
...
lumys skill install ./my-skill/

# Install from FangHub
lumys skill install web-search

# Install an OpenClaw-format skill
lumys skill install ./openclaw-skill/
```
...
```bash
lumys skill search "docker kubernetes"
```
...
Creates a directory under `~/.lumys/skills/<name>/` with:
...
3. Saves tokens to `~/.lumys/.env` with owner-only permissions.
...
# Direct setup
lumys channel setup telegram
lumys channel setup discord
lumys channel setup slack
```
...
```bash
lumys channel test telegram
```

---

### lumys channel enable

Enable a channel integration.

```
lumys channel enable <CHANNEL>
```
...
Prints the contents of `~/.lumys/config.toml` with the file path as a header comment.
...
```bash
lumys config get default_model.provider
# groq

lumys config get api_listen
# 127.0.0.1:4200

lumys config get memory.decay_rate
# 0.05
```
...
```bash
lumys config set default_model.provider anthropic
lumys config set default_model.model claude-sonnet-4-20250514
lumys config set api_listen "0.0.0.0:4200"
```

---

### lumys config set-key

Save an LLM provider API key to `~/.lumys/.env`.

```
lumys config set-key <PROVIDER>
```
...
- Saves to `~/.lumys/.env` as `<PROVIDER_NAME>_API_KEY=<value>`.
...
```bash
lumys config set-key groq
# Paste your groq API key: gsk_...
# [ok] Saved GROQ_API_KEY to ~/.lumys/.env
# Testing key... OK
```

---

### lumys config delete-key

Remove an API key from `~/.lumys/.env`.

```
lumys config delete-key <PROVIDER>
```
...
```bash
lumys config delete-key openai
```

---

### lumys config test-key

Test provider connectivity with the stored API key.

```
lumys config test-key <PROVIDER>
```
...
- Reads the API key from the environment (loaded from `~/.lumys/.env`).
...
- **Daemon mode:** Finds the agent by name or ID among running agents. If no agent name is given, uses the first available agent. If no agents exist, suggests `lumys agent new`.
...
lumys chat a1b2c3d4-e5f6-7890-abcd-ef1234567890
```
...
```
lumys migrate --from <FRAMEWORK> [--source-dir <PATH>] [--dry-run]
```
...
- Converts agent configurations, YAML manifests, and settings from the source framework into Lumys OS format.
- Saves imported data to `~/.lumys/`.
...
lumys migrate --from langchain --source-dir /home/user/.langchain

# Migrate from AutoGPT
lumys migrate --from autogpt
```
...
- Exposes running Lumys OS agents as MCP tools via JSON-RPC 2.0 over stdin/stdout with Content-Length framing.
- Each agent becomes a callable tool named `lumys_agent_<name>` (hyphens replaced with underscores).
...
1. **Read `daemon.json`:** On startup, the daemon writes `~/.lumys/daemon.json` containing the listen address (e.g. `127.0.0.1:4200`). The CLI reads this file to learn where the daemon is.
...
```
lumys start          # Starts daemon, writes daemon.json
                        # Other CLI instances detect daemon.json
lumys status         # Connects to daemon via HTTP
Ctrl+C                  # Daemon shuts down, daemon.json removed

lumys doctor --repair  # Cleans up stale daemon.json from crashes
```
...
Lumys OS loads `~/.lumys/.env` into the process environment on every CLI invocation. System environment variables take priority over `.env` values.
...
lumys agent spawn ./agents/custom-agent/agent.toml

# List running agents
lumys agent list

# Chat with an agent by UUID
lumys agent chat <UUID>

# Kill an agent
lumys agent kill <UUID>
```
...
```bash
# Create a workflow
lumys workflow create ./review-pipeline.json

# List workflows
lumys workflow list

# Run a workflow
lumys workflow run <WORKFLOW_ID> "Review the latest PR"
```
...
lumys trigger list --agent-id <AGENT_ID>
...
```bash
# Search FangHub
lumys skill search "code review"

# Install a skill
lumys skill install code-reviewer
...
# Remove a skill
lumys skill remove code-reviewer
```
...
lumys channel enable discord
lumys channel disable slack
```
...
lumys config set default_model.provider anthropic
lumys config set default_model.model claude-sonnet-4-20250514
lumys config set default_model.api_key_env ANTHROPIC_API_KEY

# Manage API keys
lumys config set-key anthropic
lumys config test-key anthropic
lumys config delete-key openai
...
lumys migrate --from langchain --source-dir ~/.langchain
```
...
lumys completion bash >> ~/.bashrc
lumys completion zsh > "${fpath[1]}/_lumys"
lumys completion fish > ~/.config/fish/completions/lumys.fish
```
...
The following providers are recognized by `lumys config set-key` and `lumys doctor`:
