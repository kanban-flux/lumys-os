//! Lumys OS CLI — command-line interface for the Lumys OS Agent OS.
//!
//! When a daemon is running ("`lumys start`), the CLI talks to it over HTTP.
...
use lumys_api::server::read_daemon_info;
use lumys_kernel::LumysKernel;
use lumys_types::agent::{AgentId, AgentManifest};
...
\x1b[1;36mExamples:\x1b[0m
  lumys init                 Initialize config and data directories
  lumys start                Start the kernel daemon
  lumys tui                  Launch the interactive terminal dashboard
  lumys chat                 Quick chat with the default agent
  lumys agent new coder      Spawn a new agent from a template
  lumys models list          Browse available LLM models
  lumys add github           Install the GitHub integration
  lumys doctor               Run diagnostic health checks
  lumys channel setup        Interactive channel setup wizard
  lumys cron list            List scheduled jobs
  lumys uninstall            Completely remove Lumys OS from your system

\x1b[1;36mQuick Start:\x1b[0m
  1. lumys init              Set up config + API key
  2. lumys start             Launch the daemon
  3. lumys chat              Start chatting!

\x1b[1;36mMore:\x1b[0m
  Docs:       https://lumys.sh
  Dashboard:  http://127.0.0.1:4200/ (when daemon is running)";

/// Lumys OS — the open-source Agent Operating System.
#[derive(Parser)]
#[command(
    name = "lumys",
    version,
    about = "\u{1F426} Lumys OS \u{2014} AI Agent Operating System",
    long_about = "\u{1F426} Lumys OS \u{2014} AI Agent Operating System\n\n\
                  Lightweight, Autonomous, Transformative.\n\
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize Lumys OS (create ~/.lumys/ and default config).
    Init {
        /// Quick mode: no prompts, just write config + .env (for CI/scripts).
        #[arg(long)]
        quick: bool,
    },
    /// Start the Lumys OS kernel daemon (API server + kernel).
    Start {
    ...
    /// Migrate from another agent framework to Lumys OS.
    Migrate(MigrateArgs),
    ...
    /// Completely uninstall Lumys OS from your system.
    Uninstall {
...
enum ConfigCommands {
    ...
    /// Save an API key to ~/.lumys/.env (prompts interactively).
    SetKey {
        /// Provider name (groq, anthropic, openai, gemini, deepseek, etc.).
        provider: String,
    },
    /// Remove an API key from ~/.lumys/.env.
    DeleteKey {
        /// Provider name.
        provider: String,
    },
...
fn config_log_level() -> String {
    let config_path = if let Ok(home) = std::env::var("LUMYS_HOME") {
...
/// Get the Lumys OS home directory, respecting LUMYS_HOME env var.
fn cli_lumys_home() -> std::path::PathBuf {
    if let Ok(home) = std::env::var("LUMYS_HOME") {
...
fn main() {
    ...
    // Load ~/.lumys/.env into process environment (system env takes priority).
    dotenv::load_dotenv();
    ...
    match resp {
        Ok(r) => {
            ...
            if status.is_server_error() {
                ui::error_with_fix(
                    ...
                    "Check daemon logs: ~/.lumys/tui.log",
                );
            }
            body
        }
        Err(e) => {
            ...
            if msg.contains("timed out") || msg.contains("Timeout") {
                ui::error_with_fix(
                    "Request timed out",
                    "The agent may be processing a complex request. Try again, or check `lumys status`",
                );
            } else if msg.contains("Connection refused") || msg.contains("connect") {
                ui::error_with_fix(
                    "Cannot connect to daemon",
                    "Is the daemon running? Start it with: lumys start",
                );
            } else {
                ui::error_with_fix(
                    &format!("Daemon communication error: {msg}"),
                    "Check `lumys status` or restart: lumys start",
                );
            }
            std::process::exit(1);
        }
...
fn cmd_init(quick: bool) {
    ...
    let lumys_dir = cli_lumys_home();

    // --- Ensure directories exist ---
    if !lumys_dir.exists() {
        std::fs::create_dir_all(&lumys_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", lumys_dir.display()),
                &format!("Check permissions on {}", home.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&lumys_dir);
    }

    for sub in ["data", "agents"] {
        let dir = lumys_dir.join(sub);
        if !dir.exists() {
    ...
    bundled_agents::install_bundled_agents(&lumys_dir.join("agents"));

    if quick {
        cmd_init_quick(&lumys_dir);
    } else if !std::io::IsTerminal::is_terminal(&std::io::stdin())
        || !std::io::IsTerminal::is_terminal(&std::io::stdout())
    {
        ui::hint("Non-interactive terminal detected — running in quick mode");
        ui::hint("For the interactive wizard, run: lumys init (in a terminal)");
        cmd_init_quick(&lumys_dir);
    } else {
        cmd_init_interactive(&lumys_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(lumys_dir: &std::path::Path) {
    ui::banner();
    ui::blank();

    let (provider, api_key_env, model) = detect_best_provider();

    write_config_if_missing(lumys_dir, provider, model, api_key_env);

    ui::blank();
    ui::success("Lumys OS initialized (quick mode)");
    ui::kv("Provider", provider);
    ui::kv("Model", model);
    ui::blank();
    ui::next_steps(&[
        "Start the daemon:  lumys start",
        "Chat:              lumys chat",
    ]);
}

/// Interactive 5-step onboarding wizard (ratatui TUI).
fn cmd_init_interactive(lumys_dir: &std::path::Path) {
    ...
    match init_wizard::run() {
        InitResult::Completed {
            ...
            ui::blank();
            ui::success("Lumys OS initialized!");
            ui::kv("Provider", &provider);
            ui::kv("Model", &model);
            ...
            match launch {
                ...
                LaunchChoice::Dashboard => {
                    if let Some(base) = find_daemon() {
                        ...
                        ui::error("Daemon is not running. Start it with: lumys start");
                    }
                }
                LaunchChoice::Chat => {
...
/// Launch the lumys-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_lumys_dir: &std::path::Path) {
    ...
    let desktop_bin = {
        ...
        #[cfg(windows)]
        let name = "lumys-desktop.exe";
        #[cfg(not(windows))]
        let name = "lumys-desktop";
    ...
    match desktop_bin {
        Some(ref path) if path.exists() => {
            ui::success("Launching Lumys OS Desktop...");
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    ui::hint("Try: lumys dashboard");
                }
            }
        }
        _ => {
            ui::error("Desktop app not found.");
            ui::hint("Install it with: cargo install lumys-desktop");
            ...
            if let Some(base) = find_daemon() {
                ...
                ui::hint("Daemon is not running. Start it with: lumys start");
...
fn write_config_if_missing(
    lumys_dir: &std::path::Path,
    provider: &str,
    model: &str,
    api_key_env: &str,
) {
    let config_path = lumys_dir.join("config.toml");
    if config_path.exists() {
        ui::check_ok(&format!("Config already exists: {}", config_path.display()));
    } else {
        let default_config = format!(
            r#"# Lumys OS Agent OS configuration
# See https://github.com/ENI-Ethereal-Nexus-Institute/lumys-os for documentation

# For Docker, change to "0.0.0.0:4200" or set LUMYS_LISTEN env var.
...
fn cmd_start(config: Option<PathBuf>, yolo: bool) {
    if let Some(base) = find_daemon() {
        ui::error_with_fix(
            ...
            "Use `lumys status` to check it, or stop it first",
        );
        std::process::exit(1);
    }
    ...
    rt.block_on(async {
        let mut kernel_config = lumys_kernel::config::load_config(config.as_deref());
        if yolo {
            kernel_config.approval.auto_approve = true;
            kernel_config.approval.apply_shorthands();
        }
        let kernel = match LumysKernel::boot_with_config(kernel_config) {
            Ok(k) => k,
            Err(e) => {
                boot_kernel_error(&e);
                std::process::exit(1);
        ...
        ui::kv("Model", &model);
        ui::blank();
        ui::hint("Open the dashboard in your browser, or run `lumys chat`");
        ui::hint("Press Ctrl+C to stop the daemon");
        ui::blank();

        if let Err(e) =
            lumys_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
        ...
        ui::blank();
        println!("  Lumys OS daemon stopped.");
    });
}

/// Read the api_key from ~/.lumys/config.toml (if any).
///
...
fn read_api_key() -> Option<String> {
    ...
    let config_path = cli_lumys_home().join("config.toml");
    ...
    // 2. Fall back to LUMYS_API_KEY env var
    if let Ok(key) = std::env::var("LUMYS_API_KEY") {
    ...
    match find_daemon() {
        ...
        None => {
            ui::warn_with_fix(
                ...
                "Is it running? Check with: lumys status",
            );
        }
    }
}
...
fn cmd_agent_spawn(config: Option<PathBuf>, manifest_path: PathBuf) {
    if !manifest_path.exists() {
        ui::error_with_fix(
            ...
            "Use "`lumys agent new` to spawn from a template instead",
        );
        std::process::exit(1);
    }
    ...
    if let Some(base) = find_daemon() {
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                println!("  For persistent agents, use "`lumys start` first.");
            }
            Err(e) => {
    ...
    match field {
        "model" => {
            if let Some(base) = find_daemon() {
                ...
                eprintln!("No running daemon found. Start one with: lumys start");
                std::process::exit(1);
            }
        }
        _ => {
...
fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    if all_templates.is_empty() {
        ui::error_with_fix(
            ...
            "Run "`lumys init` to set up the agents directory",
        );
        std::process::exit(1);
    }
        ...
        Some(ref name) => match all_templates.iter().find(|t| t.name == *name) {
            ...
            None => {
                ui::error_with_fix(
                    ...
                    "Run "`lumys agent new` to see available templates",
                );
                std::process::exit(1);
            }
        },
...
fn spawn_template_agent(config: Option<PathBuf>, template: &templates::AgentTemplate) {
    if let Some(base) = find_daemon() {
        ...
        if let Some(id) = body["agent_id"].as_str() {
            ...
            ui::blank();
            ui::hint(&format!("Chat: lumys chat {}", template.name));
        } else {
            ui::error(&format!(
                "Failed to spawn: {}",
                body["error"].as_str().unwrap_or("Unknown error")
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                ui::hint("For persistent agents, use "`lumys start` first");
            }
            Err(e) => {
...
fn cmd_status(config: Option<PathBuf>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("Lumys OS Daemon Status");
        ui::blank();
        ...
        ui::section("Lumys OS Status (in-process)");
        ui::blank();
        ...
        ui::blank();
        ui::hint("Run "`lumys start` to launch the daemon");
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(_h) = &home {
        ...
        // --- Check 1: Lumys OS directory ---
        if lumys_dir.exists() {
            if !json {
                ui::check_ok(&format!("Lumys OS directory: {}", lumys_dir.display()));
            }
            checks.push(serde_json::json!({"check": "lumys_dir", "status": "ok", "path": lumys_dir.display().to_string()}));
        } else if repair {
            if !json {
                ui::check_fail("Lumys OS directory not found.");
            }
            let answer = prompt_input("    Create it now? [Y/n] ");
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                if std::fs::create_dir_all(&lumys_dir).is_ok() {
                    restrict_dir_permissions(&lumys_dir);
                    for sub in ["data", "agents"] {
                        let _ = std::fs::create_dir_all(lumys_dir.join(sub));
                    }
                    if !json {
                        ui::check_ok("Created Lumys OS directory");
                    }
                    repaired = true;
                } else {
                    if !json {
            ...
            checks.push(serde_json::json!({"check": "lumys_dir", "status": if repaired { "repaired" } else { "fail" }}));
        } else {
            if !json {
                ui::check_fail("Lumys OS directory not found. Run "`lumys init` first.");
            }
            checks.push(serde_json::json!({"check": "lumys_dir", "status": "fail"}));
            all_ok = false;
        }

        // --- Check 2: .env file exists + permissions ---
        let env_path = lumys_dir.join(".env");
        if env_path.exists() {
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
            ...
            if !json {
                ui::check_warn(
                    ".env file not found (create with: lumys config set-key <provider>)",
                );
            }
            ...
            match toml::from_str::<toml::Value>(&config_content) {
                ...
                Err(e) => {
                    if !json {
                        ...
                        ui::hint("Fix with: lumys config edit");
                    }
...
"#
                ...
                let _ = std::fs::create_dir_all(&lumys_dir);
        ...
        let api_listen = {
            let cfg_path = lumys_dir.join("config.toml");
            if cfg_path.exists() {
                std::fs::read_to_string(&cfg_path)
                    .ok()
                    .and_then(|s| toml::from_str::<lumys_types::config::KernelConfig>(&s).ok())
                    .map(|c| c.api_listen)
        ...
        if let Some(ref base) = daemon_running {
            ...
            if !json {
                ui::check_warn("Daemon not running (start with "`lumys start`)");
            }
        ...
        let daemon_json_path = lumys_dir.join("daemon.json");
        ...
        let db_path = lumys_dir.join("data").join("lumys.db");
        if db_path.exists() {
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &lumys_dir.display().to_string()])
                .output()
            {
        ...
        let agents_dir = lumys_dir.join("agents");
    ...
    if !any_key_set {
        if !json {
            ...
            ui::blank();
            ui::hint("Or run: lumys config set-key groq");
        }
        all_ok = false;
    }
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<lumys_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = lumys_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                lumys_types::config::McpTransportEntry::Stdio {
                                    command,
                                    ..
                                } => {
                                    if command.is_empty() {
                                ...
                                lumys_types::config::McpTransportEntry::Sse { url } => {
                                    if url.is_empty() {
                                        if !json {
                                            ui::check_warn(&format!(
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = cli_lumys_home().join("skills");
        let mut skill_reg = lumys_skills::registry::SkillRegistry::new(skills_dir.clone());
        ...
        for skill in &skills {
            if let Some(ref prompt) = skill.manifest.prompt_context {
                let warnings = lumys_skills::verify::SkillVerifier::scan_prompt_content(prompt);
                let has_critical = warnings.iter().any(|w| {
                    matches!(
                        w.severity,
                        lumys_skills::verify::WarningSeverity::Critical
                    )
                });
                if has_critical {
    ...
    // --- Check 14: Extension registry health ---
    {
        ...
        let mut ext_registry =
            lumys_extensions::registry::IntegrationRegistry::new(&lumys_dir);
    ...
    if json {
        ...
        println!();
        if all_ok {
            ui::success("All checks passed! Lumys OS is ready.");
            if find_daemon().is_none() {
                ui::hint("Start the daemon: lumys start");
            }
        } else if repaired {
            ui::success("Repairs applied. Re-run `lumys doctor` to verify.");
        } else {
            ui::error("Some checks failed.");
            if !repair {
                ui::hint("Run `lumys doctor --repair` to attempt auto-fix");
            }
        }
    }
}
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "lumys", &mut std::io::stdout());
}
...
fn require_daemon(command: &str) -> String {
    find_daemon().unwrap_or_else(|| {
        ui::error_with_fix(
            &format!("`lumys {command}` requires a running daemon"),
            "Start the daemon: lumys start",
        );
        ui::hint("Or try `lumys chat` which works without a daemon");
        std::process::exit(1);
    })
}

fn boot_kernel(config: Option<PathBuf>) -> LumysKernel {
    match LumysKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_migrate(args: MigrateArgs) {
    let source = match args.from {
        MigrateSourceArg::Openclaw => lumys_migrate::MigrateSource::OpenClaw,
        MigrateSourceArg::Langchain => lumys_migrate::MigrateSource::LangChain,
        MigrateSourceArg::Autogpt => lumys_migrate::MigrateSource::AutoGpt,
    };
    ...
    let source_dir = args.source_dir.unwrap_or_else(|| {
        ...
        match source {
            lumys_migrate::MigrateSource::OpenClaw => home.join(".openclaw"),
            lumys_migrate::MigrateSource::LangChain => home.join(".langchain"),
            lumys_migrate::MigrateSource::AutoGpt => home.join("Auto-GPT"),
        }
    });

    let target_dir = cli_lumys_home();
    ...
    let options = lumys_migrate::MigrateOptions {
        source,
        source_dir,
        target_dir,
    ...
    match lumys_migrate::run_migration(&options) {
        Ok(report) => {
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            ...
            if lumys_skills::openclaw_compat::detect_openclaw_skill(&source_path) {
                println!("Detected OpenClaw skill format. Converting...");
                match lumys_skills::openclaw_compat::convert_openclaw_skill(&source_path) {
                    Ok(manifest) => {
                        let dest = skills_dir.join(&manifest.skill.name);
                        // Copy skill directory
                        copy_dir_recursive(&source_path, &dest);
                        if let Err(e) = lumys_skills::openclaw_compat::write_lumys_manifest(
                            &dest, &manifest,
                        ) {
        ...
        let manifest: lumys_skills::SkillManifest =
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = lumys_skills::marketplace::MarketplaceClient::new(
            lumys_skills::marketplace::MarketplaceConfig::default(),
        );
...
fn cmd_skill_list() {
    ...
    let mut registry = lumys_skills::registry::SkillRegistry::new(skills_dir);
...
"#
    ...
    println!("  2. Test locally: lumys skill test");
    println!(
        "  3. Install: lumys skill install {}",
        skill_dir.display()
    );
}
...
fn cmd_channel_list() {
    ...
    if !config_path.exists() {
        println!("No configuration found. Run "`lumys init` first.");
        return;
    }
    ...
    println!("\nUse `lumys channel setup <channel>` to configure a channel.");
}
    ...
    match channel.as_str() {
        "telegram" => {
            ...
            match dotenv::save_env_key("TELEGRAM_BOT_TOKEN", &token) {
                Ok(()) => ui::success("Token saved to ~/.lumys/.env"),
            ...
            for (key, val) in [
                ...
                if !val.is_empty() {
                    match dotenv::save_env_key(key, val) {
                        Ok(()) => ui::success(&format!("{key} saved to ~/.lumys/.env")),
        ...
        "email" => {
            ...
            if !password.is_empty() {
                match dotenv::save_env_key("EMAIL_PASSWORD", &password) {
                    Ok(()) => ui::success("Password saved to ~/.lumys/.env"),
                    Err(_) => println!("    export EMAIL_PASSWORD=your_app_password"),
                }
            } else {
                ui::hint("Set later: lumys config set-key email (or export EMAIL_PASSWORD=...)");
            }
        ...
        "signal" => {
            ...
            if !phone.is_empty() {
                match dotenv::save_env_key("SIGNAL_PHONE", &phone) {
                    Ok(()) => ui::success("Phone saved to ~/.lumys/.env"),
        ...
        "matrix" => {
            ...
            println!("     (e.g., register @lumys-bot:matrix.org)");
            println!("  2. Obtain an access token:");
            println!("     curl -X POST https://matrix.org/_matrix/client/r0/login \\");
            println!("       -d '{{\"type\":\"m.login.password\",\"user\":\"lumys-bot\",\"password\":\"...\"}}'");
...
fn maybe_write_channel_config(channel: &str, config_block: &str) {
    ...
    if !config_path.exists() {
        ui::hint("No config.toml found. Run "`lumys init` first.");
        return;
    }
...
fn cmd_channel_test(channel: &str) {
    if let Some(base) = find_daemon() {
        ...
        eprintln!("Channel test requires a running daemon. Start with: lumys start");
        std::process::exit(1);
    }
}
...
fn cmd_channel_toggle(channel: &str, enable: bool) {
    ...
    if let Some(base) = find_daemon() {
        ...
        println!("Edit ~/.lumys/config.toml to persist this change.");
    }
}
...
fn cmd_hand_install(path: &str) {
    ...
    println!(
        "Use `lumys hand activate {}` to start it.",
        body["id"].as_str().unwrap_or("?")
    );
}
...
fn cmd_hand_list() {
    ...
    if let Some(arr) = Some(&arr_val) {
        ...
        println!("\nUse `lumys hand activate <id>` to activate a hand.");
    }
}
...
/// Spawn "`lumys start` as a detached background process.
///
...
fn cmd_config_show() {
    ...
    if !config_path.exists() {
        ...
        println!("Run "`lumys init` to create one.");
        return;
    }
...
fn cmd_config_get(key: &str) {
    ...
    if !config_path.exists() {
        ui::error_with_fix("No config file found", "Run "`lumys init` first");
        std::process::exit(1);
    }
    ...
    let table: toml::Value = toml::from_str(&content).unwrap_or_else(|e| {
        ui::error_with_fix(
            ...
            "Fix your config.toml syntax, or run "`lumys config edit`",
        );
        std::process::exit(1);
    });
...
fn cmd_config_set_key(provider: &str) {
    ...
    match dotenv::save_env_key(&env_var, &key) {
        Ok(()) => {
            ui::success(&format!("Saved {env_var} to ~/.lumys/.env"));
            // Test the key
            print!("  Testing key... ");
...
fn cmd_config_delete_key(provider: &str) {
    ...
    // Remove from vault (best-effort)
    {
        ...
        if vault_path.exists() {
            let mut vault = lumys_extensions::vault::CredentialVault::new(vault_path);
    ...
    match dotenv::remove_env_key(&env_var) {
        Ok(()) => ui::success(&format!("Removed {env_var} from ~/.lumys/.env")),
        Err(e) => {
...
fn cmd_config_test_key(provider: &str) {
    ...
    if std::env::var(&env_var).is_err() {
        ...
        ui::hint(&format!("Set it: lumys config set-key {provider}"));
        std::process::exit(1);
    }
    ...
    if test_api_key(provider, &env_var) {
        ...
        ui::hint(&format!("Update key: lumys config set-key {provider}"));
        std::process::exit(1);
    }
}
...
pub(crate) fn lumys_home() -> PathBuf {
    if let Ok(home) = std::env::var("LUMYS_HOME") {
...
// ---------------------------------------------------------------------------
// Integration commands (lumys add/remove/integrations)
// ---------------------------------------------------------------------------

fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = lumys_home();
    let mut registry = lumys_extensions::registry::IntegrationRegistry::new(&home);
    ...
    let vault = if vault_path.exists() {
        let mut v = lumys_extensions::vault::CredentialVault::new(vault_path);
        if v.unlock().is_ok() {
            Some(v)
        } else {
            None
    ...
    let mut resolver =
        lumys_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
    ...
    match lumys_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                lumys_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                lumys_extensions::IntegrationStatus::Setup => {
                    println!("{}", result.message.yellow());
                    println!("\nTo add credentials:");
                    for env in &template.required_env {
                        if env.is_secret {
                            println!("  lumys vault set {}  # {}", env.name, env.help);
...
fn cmd_integration_remove(name: &str) {
    ...
    match lumys_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        lumys_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        lumys_extensions::installer::search_integrations(&registry, q)
    } else {
        lumys_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&lumys_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                lumys_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                lumys_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                lumys_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                lumys_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                lumys_extensions::IntegrationStatus::Disabled => {
    ...
    println!("  Use `lumys add <name>` to install an integration.");
}

// ---------------------------------------------------------------------------
// Vault commands (lumys vault init/set/list/remove)
// ---------------------------------------------------------------------------
...
fn cmd_vault_set(key: &str) {
    ...
    if !vault.exists() {
        ui::error("Vault not initialized. Run: lumys vault init");
        std::process::exit(1);
    }
...
fn cmd_vault_list() {
    ...
    if !vault.exists() {
        println!("Vault not initialized. Run: lumys vault init");
        return;
    }
...
// ---------------------------------------------------------------------------
// Scaffold commands (lumys new skill/integration)
// ---------------------------------------------------------------------------
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            lumys_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
        ScaffoldKind::Integration => {
            lumys_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = lumys_runtime::model_catalog::ModelCatalog::new();
...
fn pick_model() -> String {
    ...
    let mut by_provider: std::collections::BTreeMap<
        String,
        Vec<&lumys_types::model_catalog::ModelCatalogEntry>,
    > = std::collections::BTreeMap::new();
    for m in models {
...
fn cmd_logs(lines: usize, follow: bool) {
    let log_path = cli_lumys_home().join("tui.log");
...
fn cmd_health(json: bool) {
    match find_daemon() {
        ...
        None => {
            ...
            ui::hint("Start it with: lumys start");
            std::process::exit(1);
        }
    }
}
...
fn cmd_devices_pair() {
    ...
    if let Some(qr) = body["qr_data"].as_str() {
        ...
        println!("  Scan this QR code with the Lumys OS mobile app:");
        ui::blank();
        println!("  {qr}");
        ui::blank();
...
fn cmd_system_info(json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("Lumys OS System Info");
        ui::blank();
        ...
        ui::kv("Version", env!("CARGO_PKG_VERSION"));
        ui::kv_warn("Daemon", "NOT RUNNING");
        ui::hint("Start with: lumys start");
    }
}
...
fn cmd_system_version(json: bool) {
    ...
    println!("lumys {}", env!("CARGO_PKG_VERSION"));
}
...
fn cmd_reset(confirm: bool) {
    ...
    if !confirm {
        println!("  This will delete all data in {}", lumys_dir.display());
    ...
    match std::fs::remove_dir_all(&lumys_dir) {
        Ok(()) => ui::success(&format!("Removed {}", lumys_dir.display())),
        Err(e) => {
            ui::error(&format!("Failed to remove {}: {e}", lumys_dir.display()));
            std::process::exit(1);
        }
    }
}
...
fn cmd_uninstall(confirm: bool, keep_config: bool) {
    ...
    println!(
        "  {}",
        "This will completely uninstall Lumys OS from your system."
            .bold()
            .red()
    );
    println!();
    if lumys_dir.exists() {
        if keep_config {
            ...
            println!("  • Remove {}", lumys_dir.display());
        }
    }
    ...
    if find_daemon().is_some() {
        ...
        if find_daemon().is_some() {
            if let Some(info) = read_daemon_info(&lumys_dir) {
                force_kill_pid(info.pid);
                let _ = std::fs::remove_file(lumys_dir.join("daemon.json"));
            }
        }
    }
    ...
    // Step 6: Remove ~/.lumys/ data
    if lumys_dir.exists() {
        if keep_config {
            remove_dir_except_config(&lumys_dir);
            ui::success("Removed data (kept config files)");
        } else {
            match std::fs::remove_dir_all(&lumys_dir) {
                Ok(()) => ui::success(&format!("Removed {}", lumys_dir.display())),
                Err(e) => ui::error(&format!("Failed to remove {}: {e}", lumys_dir.display())),
            }
        }
    }
    ...
    println!();
    ui::success("Lumys OS has been uninstalled. Goodbye!");
}
...
fn remove_autostart_entries(home: &std::path::Path) {
    ...
    #[cfg(target_os = "macos")]
    {
        let plist = home.join("Library/LaunchAgents/ai.lumys.desktop.plist");
        if plist.exists() {
            // Unload first
    ...
    #[cfg(target_os = "linux")]
    {
        let desktop_file = home.join(".config/autostart/Lumys OS.desktop");
        ...
        let service_file = home.join(".config/systemd/user/lumys.service");
        if service_file.exists() {
            let _ = std::process::Command::new("systemctl")
                .args(["--user", "disable", "--now", "lumys.service"])
                .output();
...
/// Remove lines from shell config files that add lumys to PATH.
#[allow(unused_variables)]
fn clean_path_entries(home: &std::path::Path, lumys_dir: &str) {
    #[cfg(not(windows))]
    {
        let shell_files = [
            home.join(".bashrc"),
        ...
        for path in &shell_files {
            ...
            let filtered: Vec<&str> = content
                .lines()
                .filter(|line| !is_lumys_path_line(line, lumys_dir))
                .collect();
    ...
    #[cfg(windows)]
    {
        // Read User PATH via PowerShell, filter out lumys entries, write back
        ...
        if let Ok(out) = output {
            if out.status.success() {
                ...
                if !current.is_empty() {
                    let dir_lower = lumys_dir.to_lowercase();
                    let filtered: Vec<&str> = current
                        .split(';')
                        .filter(|entry| {
                            let e = entry.trim().to_lowercase();
                            !e.is_empty() && !e.contains("lumys") && !e.contains(&dir_lower)
                        })
                        .collect();
...
/// Returns true if a shell config line is a lumys PATH export.
/// Must match BOTH an lumys reference AND a PATH-setting pattern.
#[cfg(any(not(windows), test))]
fn is_lumys_path_line(line: &str, lumys_dir: &str) -> bool {
    let lower = line.to_lowercase();
    let has_lumys = lower.contains("lumys") || lower.contains(&lumys_dir.to_lowercase());
    if !has_lumys {
        return false;
    }
...
/// Remove everything in ~/.lumys/ except config files.
fn remove_dir_except_config(lumys_dir: &std::path::Path) {
    let keep = ["config.toml", ".env", "secrets.env"];
    let Ok(entries) = std::fs::read_dir(lumys_dir) else {
        return;
    };
    for entry in entries.flatten() {
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("lumys-doctor-test-skills");
        let mut skill_reg = lumys_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("lumys-doctor-test-ext");
        let _ = std::fs::create_dir_all(&tmp);
        let mut ext_reg = lumys_extensions::registry::IntegrationRegistry::new(&tmp);
    ...
    fn test_doctor_config_deser_default() {
        ...
        let config = lumys_types::config::KernelConfig::default();
        let toml_str = toml::to_string_pretty(&config).unwrap();
        let parsed: lumys_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
"#;
        let config: lumys_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            lumys_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
        ...
        match &config.mcp_servers[0].transport {
            lumys_types::config::McpTransportEntry::Stdio { command, args } => {
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = lumys_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
    ...
    fn test_doctor_hook_event_variants() {
        ...
        use lumys_types::agent::HookEvent;
        let events = [
    ...
    fn test_uninstall_path_line_filter() {
        use super::is_lumys_path_line;
        let dir = "/home/user/.lumys/bin";

        // Should match: lumys PATH exports
        assert!(is_lumys_path_line(
            r#"export PATH="$HOME/.lumys/bin:$PATH""#,
            dir
        ));
        assert!(is_lumys_path_line(
            r#"export PATH="/home/user/.lumys/bin:$PATH""#,
            dir
        ));
        assert!(is_lumys_path_line(
            "set -gx PATH $HOME/.lumys/bin $PATH",
            dir
        ));
        assert!(is_lumys_path_line(
            "fish_add_path $HOME/.lumys/bin",
            dir
        ));
        ...
        // Should NOT match: lumys lines that aren't PATH-related
        assert!(!is_lumys_path_line("# lumys config", dir));
        assert!(!is_lumys_path_line("alias of=lumys", dir));
    }
}
