//! Route handlers for the Lumys OS API.
...
use axum::Json;
use dashmap::DashMap;
use lumys_kernel::triggers::{TriggerId, TriggerPattern};
use lumys_kernel::workflow::{
    ErrorMode, StepAgent, StepMode, Workflow, WorkflowId, WorkflowStep,
};
use lumys_kernel::LumysKernel;
use lumys_runtime::kernel_handle::KernelHandle;
use lumys_runtime::tool_runner::builtin_tool_definitions;
use lumys_types::agent::{AgentId, AgentIdentity, AgentManifest};
...
pub struct AppState {
    pub kernel: Arc<LumysKernel>,
    pub started_at: Instant,
    /// Optional peer registry for OFP mesh networking status.
    pub peer_registry: Option<Arc<lumys_wire::registry::PeerRegistry>>,
    /// Channel bridge manager — held behind a Mutex so it can be swapped on hot-reload.
    pub bridge_manager: tokio::sync::Mutex<Option<lumys_channels::bridge::BridgeManager>>,
    /// Live channel config — updated on every hot-reload so list_channels() reflects reality.
    pub channels_config: tokio::sync::RwLock<lumys_types::config::ChannelsConfig>,
    ...
    pub provider_probe_cache: lumys_runtime::provider_health::ProbeCache,
}
        ...
        match state.kernel.verify_signed_manifest(signed_json) {
            ...
            Err(e) => {
                ...
                state.kernel.audit_log.record(
                    "system",
                    lumys_runtime::audit::AuditAction::AuthAttempt,
...
pub async fn list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let agents: Vec<serde_json::Value> = state
        ...
        .map(|e| {
            ...
            let ready = matches!(e.state, lumys_types::agent::AgentState::Running)
                && auth_status != "missing";
...
) -> Vec<lumys_types::message::ContentBlock> {
    use base64::Engine;

    let upload_dir = std::env::temp_dir().join("lumys_uploads");
    ...
    for att in attachments {
        ...
        match std::fs::read(&file_path) {
            Ok(data) => {
                ...
                blocks.push(lumys_types::message::ContentBlock::Image {
...
pub fn inject_attachments_into_session(
    kernel: &LumysKernel,
    agent_id: AgentId,
    image_blocks: Vec<lumys_types::message::ContentBlock>,
) {
    use lumys_types::message::{Message, MessageContent, Role};
    ...
    let mut session = match kernel.memory.get_session(entry.session_id) {
        Ok(Some(s)) => s,
        _ => lumys_memory::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
        ...
        Ok(Some(session)) => {
            ...
            for m in &session.messages {
                ...
                let content = match &m.content {
                    lumys_types::message::MessageContent::Text(t) => t.clone(),
                    lumys_types::message::MessageContent::Blocks(blocks) => {
                        let mut texts = Vec::new();
                        for b in blocks {
                            match b {
                                lumys_types::message::ContentBlock::Text { text, .. } => {
                                    texts.push(text.clone());
                                }
                                lumys_types::message::ContentBlock::Image {
                                    media_type,
                                    data,
                                } => {
                                ...
                                lumys_types::message::ContentBlock::ToolUse {
                                    id,
                                    name,
                                    input,
                                    ..
                                ...
                                lumys_types::message::ContentBlock::ToolResult { .. } => {}
                                _ => {}
                            }
                        }
                        texts.join("\n")
            ...
            for m in &session.messages {
                if let lumys_types::message::MessageContent::Blocks(blocks) = &m.content {
                    for b in blocks {
                        if let lumys_types::message::ContentBlock::ToolResult {
                            tool_use_id,
                            content: result,
                            is_error,
                            ..
...
pub async fn restart_agent(
    ...
    let _ = state
        .kernel
        .registry
        .set_state(agent_id, lumys_types::agent::AgentState::Running);
...
pub async fn shutdown(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    state.kernel.audit_log.record(
        "system",
        lumys_runtime::audit::AuditAction::ConfigChange,
...
pub async fn list_profiles() -> impl IntoResponse {
    use lumys_types::agent::ToolProfile;
...
pub async fn send_message_stream(
    ...
    use futures::stream;
    use lumys_runtime::llm_driver::StreamEvent;
...
const CHANNEL_REGISTRY: &[ChannelMeta] = &[
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "user_id", label: "Bot User ID", field_type: FieldType::Text, env_var: None, required: false, placeholder: "@lumys:matrix.org", advanced: true },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "lumys_bot", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Bot Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "lumys_bot", advanced: false },
            ...
            ChannelField { key: "subreddits", label: "Subreddits", field_type: FieldType::List, env_var: None, required: false, placeholder: "lumys, rust", advanced: true },
            ChannelField { key: "default_agent", label: "Default Agent", field_type: FieldType::Text, env_var: None, required: false, placeholder: "assistant", advanced: true },
        ],
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "lumys", advanced: false },
            ChannelField { key: "channels", label: "Channels", field_type: FieldType::List, env_var: None, required: false, placeholder: "#lumys, #general", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Bot Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "lumys", advanced: false },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "topic", label: "Topic", field_type: FieldType::Text, env_var: None, required: true, placeholder: "lumys-alerts", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "lumys", advanced: false },
...
fn is_channel_configured(config: &lumys_types::config::ChannelsConfig, name: &str) -> bool {
    match name {
...
fn channel_config_values(
    config: &lumys_types::config::ChannelsConfig,
    name: &str,
...
pub async fn configure_channel(
    ...
    let home = lumys_kernel::config::lumys_home();
...
async fn send_channel_test_message(channel_name: &str, target_id: &str) -> Result<(), String> {
    ...
    let test_msg = "Lumys OS test message — your channel is connected!";
...
pub async fn list_templates() -> impl IntoResponse {
    let agents_dir = lumys_kernel::config::lumys_home().join("agents");
...
pub async fn get_agent_kv(
    ...
    let agent_id = lumys_kernel::kernel::shared_memory_agent_id();
...
pub async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let db_ok = tokio::task::spawn_blocking(move || {
        let shared_id = lumys_types::agent::AgentId(uuid::Uuid::from_bytes([
...
///
/// Returns counters and gauges for monitoring Lumys OS in production:
/// - `lumys_agents_active` — number of active agents
/// - `lumys_uptime_seconds` — seconds since daemon started
/// - `lumys_tokens_total` — total tokens consumed (per agent)
/// - `lumys_tool_calls_total` — total tool calls (per agent)
/// - `lumys_panics_total` — supervisor panic count
/// - `lumys_restarts_total` — supervisor restart count
pub async fn prometheus_metrics(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let mut out = String::with_capacity(2048);

    // Uptime
    let uptime = state.started_at.elapsed().as_secs();
    out.push_str("# HELP lumys_uptime_seconds Time since daemon started.\n");
    out.push_str("# TYPE lumys_uptime_seconds gauge\n");
    out.push_str(&format!("lumys_uptime_seconds {uptime}\n\n"));

    // Active agents
    let agents = state.kernel.registry.list();
    let active = agents
        .iter()
        .filter(|a| matches!(a.state, lumys_types::agent::AgentState::Running))
        .count();
    out.push_str("# HELP lumys_agents_active Number of active agents.\n");
    out.push_str("# TYPE lumys_agents_active gauge\n");
    out.push_str(&format!("lumys_agents_active {active}\n"));
    out.push_str("# HELP lumys_agents_total Total number of registered agents.\n");
    out.push_str("# TYPE lumys_agents_total gauge\n");
    out.push_str(&format!("lumys_agents_total {}\n\n", agents.len()));

    // Per-agent token and tool usage
    out.push_str("# HELP lumys_tokens_total Total tokens consumed (rolling hourly window).\n");
    out.push_str("# TYPE lumys_tokens_total gauge\n");
    out.push_str("# HELP lumys_tool_calls_total Total tool calls (rolling hourly window).\n");
    out.push_str("# TYPE lumys_tool_calls_total gauge\n");
    for agent in &agents {
        let name = &agent.name;
        ...
        if let Some((tokens, tools)) = state.kernel.scheduler.get_usage(agent.id) {
            out.push_str(&format!(
                "lumys_tokens_total{{agent=\"{name}\",provider=\"{provider}\",model=\"{model}\"}} {tokens}\n"
            ));
            out.push_str(&format!(
                "lumys_tool_calls_total{{agent=\"{name}\"}} {tools}\n"
            ));
        }
    }
    out.push('\n');
    ...
    // Supervisor health
    let health = state.kernel.supervisor.health();
    out.push_str("# HELP lumys_panics_total Total supervisor panics since start.\n");
    out.push_str("# TYPE lumys_panics_total counter\n");
    out.push_str(&format!("lumys_panics_total {}\n", health.panic_count));
    out.push_str("# HELP lumys_restarts_total Total supervisor restarts since start.\n");
    out.push_str("# TYPE lumys_restarts_total counter\n");
    out.push_str(&format!(
        "lumys_restarts_total {}\n\n",
        health.restart_count
    ));

    // Version info
    out.push_str("# HELP lumys_info Lumys OS version and build info.\n");
    out.push_str("# TYPE lumys_info gauge\n");
    out.push_str(&format!(
        "lumys_info{{version=\"{}\"}} 1\n",
    ...
    let skills: Vec<serde_json::Value> = registry
        ...
        .map(|s| {
            let source = match &s.manifest.source {
                Some(lumys_skills::SkillSource::ClawHub { slug, version }) => {
                    serde_json::json!({"type": "clawhub", "slug": slug, "version": version})
                }
                Some(lumys_skills::SkillSource::OpenClaw) => {
                    serde_json::json!({"type": "openclaw"})
                }
                Some(lumys_skills::SkillSource::Bundled) => {
                    serde_json::json!({"type": "bundled"})
                }
                Some(lumys_skills::SkillSource::Native) | None => {
...
pub async fn install_skill(
    ...
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let config = lumys_skills::marketplace::MarketplaceConfig::default();
    let client = lumys_skills::marketplace::MarketplaceClient::new(config);
...
pub async fn clawhub_search(
    ...
    let client = lumys_skills::clawhub::ClawHubClient::new(cache_dir);
...
pub async fn clawhub_browse(
    ...
    let sort = match params.get("sort").map(|s| s.as_str()) {
        Some("downloads") => lumys_skills::clawhub::ClawHubSort::Downloads,
        Some("stars") => lumys_skills::clawhub::ClawHubSort::Stars,
        Some("updated") => lumys_skills::clawhub::ClawHubSort::Updated,
        Some("rating") => lumys_skills::clawhub::ClawHubSort::Rating,
        _ => lumys_skills::clawhub::ClawHubSort::Trending,
    };
...
pub async fn clawhub_install(
    ...
    match client.install(&req.slug, &skills_dir).await {
        ...
        Err(e) => {
            let msg = format!("{e}");
            let status = if matches!(e, lumys_skills::SkillError::SecurityBlocked(_)) {
                StatusCode::FORBIDDEN
            } else if is_clawhub_rate_limit(&e) {
                StatusCode::TOO_MANY_REQUESTS
            } else if matches!(e, lumys_skills::SkillError::Network(_)) {
...
fn is_clawhub_rate_limit(err: &lumys_skills::SkillError) -> bool {
    matches!(err, lumys_skills::SkillError::RateLimited(_))
}

/// Convert a browse entry (nested stats/tags) to a flat JSON object for the frontend.
fn clawhub_browse_entry_to_json(
    entry: &lumys_skills::clawhub::ClawHubBrowseEntry,
) -> serde_json::Value {
    let version = lumys_skills::clawhub::ClawHubClient::entry_version(entry);
    serde_json::json!({
        "slug": entry.slug,
...
pub async fn activate_hand(
    State(state): State<Arc<AppState>>,
    Path(hand_id): Path<String>,
    body: Option<Json<lumys_hands::ActivateHandRequest>>,
) -> impl IntoResponse {
            ...
            if let Some(agent_id) = instance.agent_id {
                ...
                if let Some(entry) = entry {
                    if !matches!(
                        ...
                        lumys_types::agent::ScheduleMode::Reactive
                    ) {
...
pub async fn hand_stats(
    ...
    let shared_id = lumys_kernel::kernel::shared_memory_agent_id();
...
pub async fn hand_instance_browser(
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            lumys_runtime::browser::BrowserCommand::ReadPage,
        )
        .await
    {
        Ok(resp) if resp.success => {
            if let Some(data) = &resp.data {
                ...
                if content.len() > 2000 {
                    content = format!(
                        "{}... (truncated)",
                        lumys_types::truncate_str(&content, 2000)
                    );
                }
            }
        }
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            lumys_runtime::browser::BrowserCommand::Screenshot,
        )
        .await
    {
        Ok(resp) if resp.success => {
...
pub async fn update_budget(
    ...
    let config_ptr = &state.kernel.config as *const lumys_types::config::KernelConfig
        as *mut lumys_types::config::KernelConfig;
...
pub async fn agent_budget_status(
    ...
    let usage_store = lumys_memory::usage::UsageStore::new(state.kernel.memory.usage_conn());
...
pub async fn delete_session(
    ...
    let session_id = match id.parse::<uuid::Uuid>() {
        Ok(u) => lumys_types::agent::SessionId(u),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = lumys_types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn find_session_by_label(
    ...
    let agent_id = match agent_id_str.parse::<uuid::Uuid>() {
        Ok(u) => lumys_types::agent::AgentId(u),
        Err(_) => {
            // Try name lookup
...
pub async fn migrate_detect() -> impl IntoResponse {
    match lumys_migrate::openclaw::detect_openclaw_home() {
        Some(path) => {
            let scan = lumys_migrate::openclaw::scan_openclaw_workspace(&path);
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "detected": true,
...
pub async fn run_migrate(Json(req): Json<MigrateRequest>) -> impl IntoResponse {
    let source = match req.source.as_str() {
        "openclaw" => lumys_migrate::MigrateSource::OpenClaw,
        "langchain" => lumys_migrate::MigrateSource::LangChain,
        "autogpt" => lumys_migrate::MigrateSource::AutoGpt,
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(
    ...
    let models: Vec<serde_json::Value> = catalog
        ...
        .filter(|m| {
            ...
            if available_only {
                ...
                if let Some(p) = provider {
                    if p.auth_status == lumys_types::model_catalog::AuthStatus::Missing {
                        return false;
                    }
                }
            }
        ...
        .map(|m| {
            ...
            let available = catalog
                ...
                .map(|p| p.auth_status != lumys_types::model_catalog::AuthStatus::Missing)
                .unwrap_or(m.tier == lumys_types::model_catalog::ModelTier::Custom);
            serde_json::json!({
                "id": m.id,
...
pub async fn list_providers(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let provider_list: Vec<lumys_types::model_catalog::ProviderInfo> = {
        let catalog = state
            .kernel
            .model_catalog
            .read()
    ...
    let probe_futures: Vec<_> = local_providers
        .iter()
        .map(|(_, id, url)| {
            lumys_runtime::provider_health::probe_provider_cached(id, url, cache)
        })
        .collect();
    ...
    let mut probe_map: HashMap<usize, lumys_runtime::provider_health::ProbeResult> =
...
///
/// Persists to `~/.lumys/custom_models.json` and makes the model immediately
...
pub async fn add_custom_model(
    ...
    let entry = lumys_types::model_catalog::ModelCatalogEntry {
        id: id.clone(),
        display_name: display,
        provider: provider.clone(),
        tier: lumys_types::model_catalog::ModelTier::Custom,
        context_window,
...
pub async fn a2a_agent_card(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Some(first) = agents.first() {
        let card = lumys_runtime::a2a::build_agent_card(&first.manifest, &base_url);
        (
            StatusCode::OK,
        ...
        let card = serde_json::json!({
            "name": "lumys",
            "description": "Lumys OS Agent OS — no agents spawned yet",
...
pub async fn a2a_list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let cards: Vec<serde_json::Value> = agents
        .iter()
        .map(|entry| {
            let card = lumys_runtime::a2a::build_agent_card(&entry.manifest, &base_url);
...
pub async fn a2a_send_task(
    ...
    let task = lumys_runtime::a2a::A2aTask {
        id: task_id.clone(),
        session_id: session_id.clone(),
        status: lumys_runtime::a2a::A2aTaskStatus::Working.into(),
        messages: vec![lumys_runtime::a2a::A2aMessage {
            role: "user".to_string(),
            parts: vec![lumys_runtime::a2a::A2aPart::Text {
    ...
    match state.kernel.send_message(agent.id, &message_text).await {
        Ok(result) => {
            let response_msg = lumys_runtime::a2a::A2aMessage {
        ...
        Err(e) => {
            let error_msg = lumys_runtime::a2a::A2aMessage {
                role: "agent".to_string(),
                parts: vec![lumys_runtime::a2a::A2aPart::Text {
                    text: format!("Error: {e}"),
                }],
...
pub async fn a2a_discover_external(
    ...
    let client = lumys_runtime::a2a::A2aClient::new();
...
pub async fn mcp_http(
    ...
    let mut tools = builtin_tool_definitions();
    {
        ...
        for skill_tool in registry.all_tool_definitions() {
            tools.push(lumys_types::tool::ToolDefinition {
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn lumys_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn lumys_runtime::kernel_handle::KernelHandle>;
        let result = lumys_runtime::tool_runner::execute_tool(
            "mcp-http",
            tool_name,
            &arguments,
            Some(&kernel_handle),
    ...
    let response = lumys_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => lumys_types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
...
pub async fn get_agent_mcp_servers(
    ...
    if let Ok(mcp_tools) = state.kernel.mcp_tools.lock() {
        ...
        for tool in mcp_tools.iter() {
            if let Some(server) = lumys_runtime::mcp::extract_mcp_server(&tool.name) {
...
///
/// SECURITY: Writes to `~/.lumys/secrets.env`, sets env var in process,
...
pub async fn set_provider_key(
    ...
    let switched = if !current_has_key && current_provider != name {
        ...
        if let Some(model_id) = default_model {
            ...
            {
                let new_dm = lumys_types::config::DefaultModelConfig {
                    provider: name.clone(),
                    model: model_id,
        ...
        if needs_update {
            ...
            *guard = Some(lumys_types::config::DefaultModelConfig {
                api_key_env: env_var.clone(),
                ..base
            });
        }
...
pub async fn test_provider(
    ...
    let driver_config = lumys_runtime::llm_driver::DriverConfig {
        provider: name.clone(),
        api_key,
    ...
    match lumys_runtime::drivers::create_driver(&driver_config) {
        Ok(driver) => {
            // Send a minimal completion request to test connectivity
            let test_req = lumys_runtime::llm_driver::CompletionRequest {
                model: default_model.clone(),
                messages: vec![lumys_types::message::Message::user("Hi")],
                tools: vec![],
                max_tokens: 1,
                temperature: 0.0,
                system: None,
...
pub async fn set_provider_url(
    ...
    let probe = lumys_runtime::provider_health::probe_provider(&name, &base_url).await;
...
pub async fn create_skill(
    ...
    // Write skill.toml to ~/.lumys/skills/{name}/
    ...
    for info in registry.list_all_info() {
        ...
        let status = match &info.installed {
            ...
            Some(_) => match h.as_ref().map(|h| &h.status) {
                Some(lumys_extensions::IntegrationStatus::Ready) => "ready",
                Some(lumys_extensions::IntegrationStatus::Error(_)) => "error",
                _ => "installed",
            },
...
pub async fn add_integration(
    ...
    let install_err = {
        ...
        if registry.is_installed(&id) {
            ...
            let entry = lumys_extensions::InstalledIntegration {
                id: id.clone(),
...
/// Must match the value in `lumys-kernel/src/kernel.rs::shared_memory_agent_id()`.
...
const SCHEDULES_KEY: &str = "__lumys_schedules";
...
pub struct PatchAgentConfigRequest {
    ...
    pub fallback_models: Option<Vec<lumys_types::agent::FallbackModel>>,
}
...
pub async fn upload_file(
    ...
    let transcription = if content_type.starts_with("audio/") {
        let attachment = lumys_types::media::MediaAttachment {
            media_type: lumys_types::media::MediaType::Audio,
            mime_type: content_type.clone(),
            source: lumys_types::media::MediaSource::FilePath {
...
pub async fn serve_upload(Path(file_id): Path<String>) -> impl IntoResponse {
    ...
    let file_path = std::env::temp_dir().join("lumys_uploads").join(&file_id);
...
pub async fn list_approvals(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    approvals.extend(recent.into_iter().map(|record| {
        ...
        let status = match record.decision {
            lumys_types::approval::ApprovalDecision::Approved => "approved",
            lumys_types::approval::ApprovalDecision::Denied => "rejected",
            lumys_types::approval::ApprovalDecision::TimedOut => "expired",
        };
        serde_json::json!({
            "id": request.id,
...
pub async fn create_approval(
    ...
    use lumys_types::approval::{ApprovalRequest, RiskLevel};
...
pub async fn approve_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        lumys_types::approval::ApprovalDecision::Approved,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn reject_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        lumys_types::approval::ApprovalDecision::Denied,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn delete_cron_job(
    ...
    match uuid::Uuid::parse_str(&id) {
        Ok(uuid) => {
            let job_id = lumys_types::scheduler::CronJobId(uuid);
...
pub async fn webhook_wake(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<lumys_types::webhook::WakePayload>,
) -> impl IntoResponse {
...
pub async fn webhook_agent(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<lumys_types::webhook::AgentHookPayload>,
) -> impl IntoResponse {
    // Check if webhook triggers are enabled
    let wh_config = match &state.kernel.config.webhook_triggers {
        Some(c) if c.enabled => c,
...
pub async fn add_binding(
    State(state): State<Arc<AppState>>,
    Json(binding): Json<lumys_types::config::AgentBinding>,
) -> impl IntoResponse {
...
pub async fn pairing_request(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match state.kernel.pairing.create_pairing_request() {
        Ok(req) => {
            let qr_uri = format!("lumys://pair?token={}", req.token);
            Json(serde_json::json!({
                "token": req.token,
                "qr_uri": qr_uri,
...
pub async fn pairing_complete(
    ...
    let push_token = body
        ...
        .and_then(|v| v.as_str())
        .map(String::from);
    let device_info = lumys_kernel::pairing::PairedDevice {
...
pub async fn copilot_oauth_start() -> impl IntoResponse {
    ...
    match lumys_runtime::copilot_oauth::start_device_flow().await {
        Ok(resp) => {
...
pub async fn copilot_oauth_poll(
    ...
    match lumys_runtime::copilot_oauth::poll_device_flow(&device_code).await {
        lumys_runtime::copilot_oauth::DeviceFlowStatus::Pending => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "pending"})),
        ),
        lumys_runtime::copilot_oauth::DeviceFlowStatus::Complete { access_token } => {
        ...
        lumys_runtime::copilot_oauth::DeviceFlowStatus::SlowDown { new_interval } => {
            // Update interval
        ...
        lumys_runtime::copilot_oauth::DeviceFlowStatus::Expired => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
                Json(serde_json::json!({"status": "expired"})),
        ...
        lumys_runtime::copilot_oauth::DeviceFlowStatus::AccessDenied => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
        ...
        lumys_runtime::copilot_oauth::DeviceFlowStatus::Error(e) => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "error", "error": e})),
        ),
    }
...
pub async fn comms_topology(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    use lumys_types::comms::{EdgeKind, TopoEdge, TopoNode, Topology};
    ...
    for event in &events {
        if let lumys_types::event::EventPayload::Message(_) = &event.payload {
            if let lumys_types::event::EventTarget::Agent(target_id) = &event.target {
...
fn filter_to_comms_event(
    event: &lumys_types::event::Event,
    agents: &[lumys_types::agent::AgentEntry],
) -> Option<lumys_types::comms::CommsEvent> {
    use lumys_types::comms::{CommsEvent, CommsEventKind};
    use lumys_types::event::{EventPayload, EventTarget, LifecycleEvent};
    ...
    match &event.payload {
        EventPayload::Message(msg) => {
            ...
            Some(CommsEvent {
                ...
                detail: lumys_types::truncate_str(&msg.content, 200).to_string(),
            })
        }
...
fn audit_to_comms_event(
    entry: &lumys_runtime::audit::AuditEntry,
    agents: &[lumys_types::agent::AgentEntry],
) -> Option<lumys_types::comms::CommsEvent> {
    use lumys_types::comms::{CommsEvent, CommsEventKind};
        ...
        agents
            ...
            .unwrap_or_else(|| {
                if id.is_empty() || id == "system" {
                    ...
                    lumys_types::truncate_str(id, 12).to_string()
                }
            })
    };
    ...
    let (kind, detail, target_label) = match action_str.as_str() {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!(
                        ...
                        in_tok,
                        out_tok,
                        lumys_types::truncate_str(&entry.outcome, 80)
                    )
                }
            } else if entry.outcome != "ok" {
                format!(
                    "{} — {}",
                    lumys_types::truncate_str(&entry.detail, 80),
                    lumys_types::truncate_str(&entry.outcome, 80)
                )
            } else {
                lumys_types::truncate_str(&entry.detail, 200).to_string()
            };
        ...
        "AgentSpawn" => (
            ...
            format!(
                "Agent spawned: {}",
                lumys_types::truncate_str(&entry.detail, 100)
            ),
            "",
        ),
        "AgentKill" => (
...
pub async fn comms_events(
    ...
    let mut comms_events: Vec<lumys_types::comms::CommsEvent> = bus_events
        .iter()
...
pub async fn comms_send(
    State(state): State<Arc<AppState>>,
    Json(req): Json<lumys_types::comms::CommsSendRequest>,
) -> impl IntoResponse {
    // Validate from agent exists
    let from_id: lumys_types::agent::AgentId = match req.from_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let to_id: lumys_types::agent::AgentId = match req.to_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn comms_task(
    State(state): State<Arc<AppState>>,
    Json(req): Json<lumys_types::comms::CommsTaskRequest>,
) -> impl IntoResponse {
...
pub async fn auth_login(
    ...
    let cookie =
        format!("lumys_session={token}; Path=/; HttpOnly; SameSite=Strict; Max-Age={ttl_secs}");
...
pub async fn auth_logout() -> impl IntoResponse {
    let cookie = "lumys_session=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0";
    (
        StatusCode::OK,
...
mod channel_config_tests {
    ...
    fn test_is_channel_configured_wecom_none() {
        let config = lumys_types::config::ChannelsConfig::default();
        assert!(!is_channel_configured(&config, "wecom"));
    }

    #[test]
    fn test_is_channel_configured_wecom_some() {
        let mut config = lumys_types::config::ChannelsConfig::default();
        config.wecom = Some(lumys_types::config::WeComConfig {
            ...
            overrides: lumys_types::config::ChannelOverrides::default(),
        });
        assert!(is_channel_configured(&config, "wecom"));
    }
