...
Lumys OS is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).

```
lumys-cli            CLI interface, daemon auto-detect, MCP server
    |
lumys-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
lumys-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
lumys-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- lumys-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
    +-- lumys-channels   40 channel adapters, bridge, formatter, rate limiter
    +-- lumys-wire       OFP peer-to-peer networking with HMAC-SHA256 auth
    +-- lumys-migrate    Migration engine (OpenClaw YAML->TOML)
    +-- lumys-skills     60 bundled skills, FangHub marketplace, ClawHub client
    |
lumys-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
lumys-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
When `Lumys OSKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:

```
1. Load configuration
   - Read ~/.lumys/config.toml (or specified path)
   - Apply #[serde(default)] defaults for missing fields
   - Validate config and log warnings (missing API keys, etc.)

2. Create data directory
   - Ensure ~/.lumys/data/ exists

3. Initialize memory substrate
   - Open SQLite database (lumys.db)
...
The memory substrate (`lumys-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`lumys-runtime`) provides a unified interface for all LLM providers:
...
pub trait LlmDriver: Send + Sync {
    ...
    ) -> Result<LlmResponse, Lumys OSError>;
...
The `ModelCatalog` (`lumys-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
Lumys OS implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
`taint.rs` in `lumys-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`lumys-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`lumys-skills`) provides 60 bundled skills and supports external skill installation.
...
- **FangHub**: Native Lumys OS marketplace (`FangHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-Lumys OS tool name mappings in `tool_compat.rs`.
...
Lumys OS implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes Lumys OS's 23 built-in tools via the MCP protocol. Enables external tools to use Lumys OS as a tool provider.
...
The Lumys OS Protocol (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`lumys-desktop`) wraps the full Lumys OS stack in a native Tauri 2.0 application.
...
+-------------------------------------------------------------------+
    ...
    v                        v                   v         v
+------------------+  +--------------+  +--------+  +-----------+
| lumys-runtime |  | lumys-    |  | open-  |  | lumys- |
