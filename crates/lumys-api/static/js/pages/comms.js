// Lumys OS Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          Lumys OSAPI.get('/api/comms/topology'),
          Lumys OSAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      var self = this;
      var url = Lumys OSAPI.baseUrl + '/api/comms/events/stream';
      if (Lumys OSAPI.apiKey) url += '?token=' + encodeURIComponent(Lumys OSAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await Lumys OSAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await Lumys OSAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        Lumys OSToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        Lumys OSToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await Lumys OSAPI.post('/api/comms/task', body);
        Lumys OSToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        Lumys OSToast.error(e.message || 'Task failed');
      }
