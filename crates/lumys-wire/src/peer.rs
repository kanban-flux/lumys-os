//! PeerNode — TCP server and client for the Lumys OS Wire Protocol.
//!
//! A [`PeerNode`] binds a local TCP listener and accepts incoming connections
//! from other Lumys OS kernels. It also connects outward to known peers. Each
    ...
    pub fn check_and_record(&self, nonce: &str) -> Result<(), String> {
        ...
        if self.seen.contains_key(nonce) {
            return Err(format!(
                ...
                lumys_types::truncate_str(nonce, 16)
            ));
        }
...
impl Default for PeerConfig {
    fn default() -> Self {
        Self {
            ...
            node_name: "lumys-node".to_string(),
...
mod tests {
    ...
    fn test_peer_config_default() {
        ...
        assert_eq!(config.node_name, "lumys-node");
