//! Shared tool name mappings between OpenClaw and Lumys OS.
//!
//! These mappings are used by both the migration engine and the skill system
//! to normalize OpenClaw tool names into Lumys OS equivalents.

/// Map an OpenClaw tool name to its Lumys OS equivalent.
///
/// Returns `None` if the name has no known mapping (may already be
/// an Lumys OS tool name — check with [`is_known_lumys_tool`]).
...
/// Normalize a tool name to its canonical Lumys OS form.
///
/// If the name is already a known Lumys OS tool, returns it as-is.
...
pub fn normalize_tool_name(name: &str) -> &str {
    if is_known_lumys_tool(name) {
        return name;
    }
    map_tool_name(name).unwrap_or(name)
}

/// Check if a tool name is a known Lumys OS built-in tool.
pub fn is_known_lumys_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    fn test_normalize_tool_name() {
        // Known Lumys OS tools pass through unchanged
    ...
    fn test_is_known_lumys_tool() {
        ...
        for tool in &known {
            assert!(is_known_lumys_tool(tool), "Expected {tool} to be known");
        }

        // Unknown
        assert!(!is_known_lumys_tool("unknown"));
        assert!(!is_known_lumys_tool("Read"));
        assert!(!is_known_lumys_tool("Bash"));
    }
}
