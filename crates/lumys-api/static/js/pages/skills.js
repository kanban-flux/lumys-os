// Lumys OS Skills Page — OpenClaw/ClawHub ecosystem + local skills + MCP servers
'use strict';
...
function skillsPage() {
  return {
    ...
    async loadSkills() {
      ...
      this.loadError = '';
      try {
        var data = await Lumys OSAPI.get('/api/skills');
    ...
    async searchClawHub() {
      ...
      this.clawhubError = '';
      try {
        var data = await Lumys OSAPI.get('/api/clawhub/search?q=' + encodeURIComponent(this.clawhubSearch.trim()) + '&limit=20');
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20');
    ...
    async loadMoreClawHub() {
      ...
      this.clawhubLoading = true;
      try {
        var data = await Lumys OSAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20&cursor=' + encodeURIComponent(this.clawhubNextCursor));
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug));
        this.skillDetail = data;
      } catch(e) {
        Lumys OSToast.error('Failed to load skill details');
      }
    ...
    async viewSkillCode(slug) {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug) + '/code');
        this.skillCode = data.code || '';
        this.skillCodeFilename = data.filename || 'source';
        this.showSkillCode = true;
      } catch(e) {
        Lumys OSToast.error('Could not load skill source code');
      }
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await Lumys OSAPI.post('/api/clawhub/install', { slug: slug });
        this.installResult = data;
        if (data.warnings && data.warnings.length > 0) {
          Lumys OSToast.success('Skill "' + data.name + '" installed with ' + data.warnings.length + ' warning(s)');
        } else {
          Lumys OSToast.success('Skill "' + data.name + '" installed successfully');
        }
        ...
        if (msg.includes('already_installed')) {
          Lumys OSToast.error('Skill is already installed');
        } else if (msg.includes('SecurityBlocked')) {
          Lumys OSToast.error('Skill blocked by security scan');
        } else {
          Lumys OSToast.error('Install failed: ' + msg);
        }
      }
    ...
    uninstallSkill: function(name) {
      var self = this;
      Lumys OSToast.confirm('Uninstall Skill', 'Uninstall skill "' + name + '"? This cannot be undone.', async function() {
        try {
          await Lumys OSAPI.post('/api/skills/uninstall', { name: name });
          Lumys OSToast.success('Skill "' + name + '" uninstalled');
          await self.loadSkills();
        } catch(e) {
          Lumys OSToast.error('Failed to uninstall skill: ' + e.message);
        }
      });
    },
    ...
    async createDemoSkill(skill) {
      try {
        await Lumys OSAPI.post('/api/skills/create', {
          name: skill.name,
          description: skill.description,
          runtime: 'prompt_only',
          prompt_context: skill.prompt_context || skill.description
        });
        Lumys OSToast.success('Skill "' + skill.name + '" created');
        this.tab = 'installed';
        await this.loadSkills();
      } catch(e) {
        Lumys OSToast.error('Failed to create skill: ' + e.message);
      }
    },
    ...
    async loadMcpServers() {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/mcp/servers');
