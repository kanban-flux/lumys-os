# Migrating to Lumys OS

This guide covers migrating from OpenClaw (and other frameworks) to Lumys OS. The migration engine handles config conversion, agent import, memory transfer, channel re-configuration, and skill scanning.
...
This auto-detects your OpenClaw workspace at `~/.openclaw/` and imports everything into `~/.lumys/`.

### Options

```bash
# Specify a custom source directory
lumys migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.lumys/` with a summary of everything that was imported, skipped, or needs manual attention.
...
```bash
lumys migrate --from langchain   # Coming soon
lumys migrate --from autogpt     # Coming soon
```
...
| Item | Source (OpenClaw) | Destination (Lumys OS) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.lumys/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.lumys/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.lumys/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.lumys/channels_import.toml` | Automated (manual merge) |
...
Channel configurations (Telegram, Discord, Slack) are exported to a `channels_import.toml` file. You must manually merge the `[channels]` section into your `~/.lumys/config.toml`.
...
```bash
lumys skill install <skill-name-or-path>
```

Lumys OS automatically detects OpenClaw-format skills and converts them during installation.
...
### 1. Initialize Lumys OS

```bash
lumys init
```

This creates `~/.lumys/` with a default `config.toml`.
...
**Lumys OS** (`~/.lumys/agents/coder/agent.toml`):
```toml
name = "coder"
version = "0.1.0"
description = "A coding assistant"
...
**Lumys OS** (add to `~/.lumys/config.toml`):
```toml
[channels.telegram]
bot_token_env = "TELEGRAM_BOT_TOKEN"
default_agent = "coder"
...
Copy any `MEMORY.md` files from OpenClaw agents to Lumys OS agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.lumys/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | Lumys OS |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.lumys/config.toml` |
...
Tools were renamed between OpenClaw and Lumys OS for consistency. The migration engine handles this automatically.

| OpenClaw Tool | Lumys OS Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Profile | Lumys OS Tools |
|------------------|----------------|
...
| OpenClaw Name | Lumys OS Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (Lumys OS Protocol) peer-to-peer |
...
```bash
lumys migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
lumys skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.lumys/channels_import.toml
# Copy the [channels.*] sections into ~/.lumys/config.toml
```
