// Lumys OS Overview Dashboard — Landing page with system stats + provider status
'use strict';
...
function overviewPage() {
  return {
    ...
    async loadHealth() {
      try {
        this.health = await Lumys OSAPI.get('/api/health');
      } catch(e) { this.health = { status: 'unreachable' }; }
    },

    async loadStatus() {
      try {
        this.status = await Lumys OSAPI.get('/api/status');
    ...
    async loadAudit() {
      try {
        var data = await Lumys OSAPI.get('/api/audit/recent?n=8');
