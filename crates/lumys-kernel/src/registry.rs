...
use dashmap::DashMap;
use lumys_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> LumysResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(LumysError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> LumysResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: lumys_types::agent::SessionId,
    ) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: lumys_types::agent::AgentIdentity,
    ) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_fallback_models(
        &self,
        id: AgentId,
        fallback_models: Vec<lumys_types::agent::FallbackModel>,
    ) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> LumysResult<()> {
        if let Some(existing_id) = self.name_index.get(&new_name).as_deref().copied() {
            if existing_id != id {
                return Err(LumysError::AgentAlreadyExists(new_name));
            }
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> LumysResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| LumysError::AgentNotFound(id.to_string()))?;
