...
ARG LUMYS_SMOKE_FULL=0
RUN if [ "$LUMYS_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$LUMYS_SMOKE_FULL" = "1" ] && [ -f "$HOME/.lumys/bin/lumys" ]; then \
        $HOME/.lumys/bin/lumys --version && \
        echo "PASS: lumys binary works"; \
    else \
