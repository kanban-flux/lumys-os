//! Channel bridge — connects channel adapters to the Lumys OS kernel.
//!
//! Defines `ChannelBridgeHandle` (implemented by lumys-api on the kernel) and
...
use lumys_types::config::{ChannelOverrides, DmPolicy, GroupPolicy, OutputFormat};
use lumys_types::message::ContentBlock;
use std::sync::Arc;
...
///
/// Defined here to avoid circular deps (lumys-channels can't depend on lumys-kernel).
/// Implemented in lumys-api on the actual kernel.
#[async_trait]
...
async fn dispatch_message(
    ...
    if router.has_broadcast(&message.sender.platform_id) {
        ...
        if !targets.is_empty() {
            ...
            match strategy {
                lumys_types::config::BroadcastStrategy::Parallel => {
                ...
                lumys_types::config::BroadcastStrategy::Sequential => {
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
    ...
    let agent_id = router.resolve(
        ...
        message.sender.lumys_user.as_deref(),
    );
...
async fn handle_command(
    ...
    match name {
        "start" => {
            ...
            let mut msg = "Welcome to Lumys OS! I connect you to AI agents.\n\nAvailable agents:\n"
                .to_string();
            if agents.is_empty() {
