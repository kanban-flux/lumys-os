// Lumys OS App — Alpine.js init, hash router, global store
'use strict';
...
document.addEventListener('alpine:init', function() {
  ...
  var savedKey = localStorage.getItem('lumys-api-key');
  if (savedKey) Lumys OSAPI.setAuthToken(savedKey);
  ...
  Alpine.store('app', {
    ...
    pendingAgent: null,
    focusMode: localStorage.getItem('lumys-focus') === 'true',
    ...
    toggleFocusMode() {
      ...
      localStorage.setItem('lumys-focus', this.focusMode);
    },

    async refreshAgents() {
      try {
        var agents = await Lumys OSAPI.get('/api/agents');
    ...
    async refreshApprovals() {
      try {
        var data = await Lumys OSAPI.get('/api/approvals');
        ...
        var signature = pending
          ...
          .sort()
          .join(',');
        if (pending.length > 0 && signature !== this.lastPendingApprovalSignature && typeof Lumys OSToast !== 'undefined') {
          Lumys OSToast.warn('An agent is waiting for approval. Open Approvals to review.');
        }
    ...
    async checkStatus() {
      try {
        var s = await Lumys OSAPI.get('/api/status');
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        console.warn('[Lumys OS] Status check failed:', e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem('lumys-onboarded')) return;
      try {
        var config = await Lumys OSAPI.get('/api/config');
    ...
    dismissOnboarding() {
      ...
      localStorage.setItem('lumys-onboarded', 'true');
    },

    async checkAuth() {
      try {
        // First check if session-based auth is configured
        var authInfo = await Lumys OSAPI.get('/api/auth/check');
      ...
      try {
        await Lumys OSAPI.get('/api/tools');
        this.showAuthPrompt = false;
      } catch(e) {
        if (e.message && (e.message.indexOf('Not authorized') >= 0 || e.message.indexOf('401') >= 0 || e.message.indexOf('Missing Authorization') >= 0 || e.message.indexOf('Unauthorized') >= 0)) {
          var saved = localStorage.getItem('lumys-api-key');
          if (saved) {
            Lumys OSAPI.setAuthToken('');
            localStorage.removeItem('lumys-api-key');
          }
          this.showAuthPrompt = true;
        }
      }
    ...
    submitApiKey(key) {
      ...
      Lumys OSAPI.setAuthToken(key.trim());
      localStorage.setItem('lumys-api-key', key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
    ...
    async sessionLogin(username, password) {
      try {
        var result = await Lumys OSAPI.post('/api/auth/login', { username: username, password: password });
        if (result.status === 'ok') {
          this.sessionUser = result.username;
          this.showAuthPrompt = false;
          this.refreshAgents();
        } else {
          Lumys OSToast.error(result.error || 'Login failed');
        }
      } catch(e) {
        Lumys OSToast.error(e.message || 'Login failed');
      }
    },

    async sessionLogout() {
      try {
        await Lumys OSAPI.post('/api/auth/logout');
      } catch(e) { /* ignore */ }
      this.sessionUser = null;
      this.showAuthPrompt = true;
    },
...
function app() {
  return {
    page: 'agents',
    themeMode: localStorage.getItem('lumys-theme-mode') || 'system',
    theme: (() => {
      var mode = localStorage.getItem('lumys-theme-mode') || 'system';
      if (mode === 'system') return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
      return mode;
    })(),
    sidebarCollapsed: localStorage.getItem('lumys-sidebar') === 'collapsed',
    ...
    init() {
      ...
      Lumys OSAPI.onConnectionChange(function(state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem('lumys-theme-mode', mode);
      if (mode === 'system') {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('lumys-sidebar', this.sidebarCollapsed ? 'collapsed' : 'expanded');
    },
    ...
    async pollStatus() {
      ...
      this.wsConnected = Lumys OSAPI.isWsConnected();
    }
  };
}
