...
Common issues, diagnostics, and answers to frequently asked questions about Lumys OS.
...
Lumys OS uses `tracing` for structured logging. Set the log level via environment:

```bash
RUST_LOG=info lumys start          # Default
RUST_LOG=debug lumys start         # Verbose
RUST_LOG=lumys=debug lumys start  # Only Lumys OS debug, deps at info
```
...
### `lumys` command not found after install
...
**Common causes**:
- No API key provided: `docker run -e GROQ_API_KEY=... ghcr.io/ENI-Ethereal-Nexus-Institute/lumys-os`
...
**Fix**: Run `lumys init` to create the default config:
```bash
lumys init
```

This creates `~/.lumys/config.toml` with sensible defaults.
...
```bash
RUST_LOG=lumys_channels=debug lumys start
```
...
**Automatic protection**: Lumys OS has a built-in loop guard:
...
**Check**:
1. TOML manifest is valid: `lumys agent spawn --dry-run manifest.toml`
...
2. Set the model to `lumys:agent-name` (e.g., `lumys:coder`)
...
3. Try deleting `~/.lumys/daemon.json` and restarting
...
If slower:
- Check database size (`~/.lumys/data/lumys.db`)
...
1. Add the channel config to `~/.lumys/config.toml` under `[channels]`
2. Set the required environment variables (tokens, secrets)
3. Restart the daemon

### How do I update Lumys OS?

```bash
# From source
cd lumys && git pull && cargo install --path crates/lumys-cli
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.lumys/data/lumys.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.lumys/config.toml` (configuration)
- `~/.lumys/data/lumys.db` (all agent data, memory, sessions)
- `~/.lumys/skills/` (installed skills)

### How do I reset everything?

```bash
rm -rf ~/.lumys
lumys init  # Start fresh
```

### Can I run Lumys OS without an internet connection?
...
### What's the difference between Lumys OS and OpenClaw?

| Aspect | Lumys OS | OpenClaw |
|--------|----------|----------|
...
Lumys OS can import OpenClaw configs: `lumys migrate --from openclaw`
...
```bash
RUST_LOG=lumys_runtime=debug,lumys_channels=info lumys start
```

### Can I use Lumys OS as a library?

Yes. Each crate is independently usable:
```toml
[dependencies]
lumys-runtime = { path = "crates/lumys-runtime" }
lumys-memory = { path = "crates/lumys-memory" }
```

The `lumys-kernel` crate assembles everything, but you can use individual crates for custom integrations.
...
### How do I run Lumys OS in Docker?

```bash
docker run -d --name lumys \
  -e GROQ_API_KEY=your_key_here \
  -p 4200:4200 \
  ghcr.io/eni-ethereal-nexus/lumys-os:latest
```

### How do I protect the dashboard with a password?

Lumys OS doesn't have built-in login. Use a reverse proxy with basic auth:
...
Not yet — each channel type currently supports one bot. Multi-bot routing is tracked as a feature request (#586). As a workaround, run multiple Lumys OS instances on different ports with different configs.

### Claude Code integration shows errors

Add to `~/.lumys/config.toml`:
```toml
[claude_code]
