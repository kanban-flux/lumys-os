...
pub struct ChannelUser {
    ...
    /// Optional mapping to an Lumys OS user identity.
    pub lumys_user: Option<String>,
}
...
// Re-export policy/format types from lumys-types for convenience.
pub use lumys_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the Lumys OS kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = lumys_types::truncate_str(remaining, max_len).len();
