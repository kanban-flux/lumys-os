...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> LumysResult<()> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn query_hourly(&self, agent_id: AgentId) -> LumysResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_daily(&self, agent_id: AgentId) -> LumysResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_monthly(&self, agent_id: AgentId) -> LumysResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_global_hourly(&self) -> LumysResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_global_monthly(&self) -> LumysResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> LumysResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> LumysResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
        ...
        for row in rows {
            results.push(row.map_err(|e| LumysError::Memory(e.to_string()))?);
        }
        Ok(results)
    }

    /// Query daily usage breakdown for the last N days.
    pub fn query_daily_breakdown(&self, days: u32) -> LumysResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_first_event_date(&self) -> LumysResult<Option<String>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn query_today_cost(&self) -> LumysResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
    ...
    pub fn cleanup_old(&self, days: u32) -> LumysResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LumysError::Internal(e.to_string()))?;
