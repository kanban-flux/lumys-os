...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p lumys-kernel --test multi_agent_test -- --nocapture
...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("lumys-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = LumysKernel::boot_with_config(test_config()).expect("Kernel should boot");
...
"#,
    ...
    println!("\n{}", "=".repeat(60));
    println!("  LUMYS MULTI-AGENT FLEET TEST");
