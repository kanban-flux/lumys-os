// Lumys OS Logs Page — Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    startStreaming: function() {
      ...
      var sep = '?';
      var token = Lumys OSAPI.getToken();
    ...
    async fetchLogs() {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/audit/recent?n=200');
    ...
    exportLogs: function() {
      ...
      a.href = url;
      a.download = 'lumys-logs-' + new Date().toISOString().slice(0, 10) + '.txt';
      a.click();
    ...
    async verifyChain() {
      try {
        var data = await Lumys OSAPI.get('/api/audit/verify');
        this.chainValid = data.valid === true;
        if (this.chainValid) {
          Lumys OSToast.success('Audit chain verified — ' + (data.entries || 0) + ' entries valid');
        } else {
          Lumys OSToast.error('Audit chain broken!');
        }
      } catch(e) {
        this.chainValid = false;
        Lumys OSToast.error('Chain verification failed: ' + e.message);
      }
    },
