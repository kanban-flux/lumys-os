...
pub fn default_client_ids() -> HashMap<&'static str, &'static str> {
    ...
    m.insert("google", "lumys-google-client-id");
    m.insert("github", "lumys-github-client-id");
    m.insert("microsoft", "lumys-microsoft-client-id");
    m.insert("slack", "lumys-slack-client-id");
    m
}

/// Resolve OAuth client IDs with config overrides applied on top of defaults.
pub fn resolve_client_ids(config: &lumys_types::config::OAuthConfig) -> HashMap<String, String> {
...
mod tests {
    ...
    fn resolve_client_ids_uses_defaults() {
        let config = lumys_types::config::OAuthConfig::default();
        let ids = resolve_client_ids(&config);
        assert_eq!(ids["google"], "lumys-google-client-id");
        assert_eq!(ids["github"], "lumys-github-client-id");
    }

    #[test]
    fn resolve_client_ids_applies_overrides() {
        let config = lumys_types::config::OAuthConfig {
        ...
        assert_eq!(ids["github"], "lumys-github-client-id"); // default
        assert_eq!(ids["microsoft"], "my-msft-id");
        assert_eq!(ids["slack"], "lumys-slack-client-id"); // default
    }
}
