...
Lumys OS ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
lumys spawn orchestrator
lumys spawn coder
lumys spawn --template agents/writer/agent.toml
```
...
> General-purpose assistant. The default Lumys OS agent for everyday tasks, questions, and conversations.
...
lumys spawn coder --name "backend-coder"

# Spawn from a TOML file path
lumys spawn --template agents/custom/my-agent.toml

# List running agents
lumys agents

# Send a message
lumys message <agent-id> "Write a function to parse TOML files"
