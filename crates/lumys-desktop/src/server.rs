...
//!
//! Boots the Lumys OS kernel, binds to a random localhost port, and runs the
//! API server on a background thread with its own tokio runtime.

use lumys_api::server::build_router;
...
impl ServerHandle {
    ...
    pub fn shutdown(mut self) {
            ...
            .is_ok()
        {
            ...
            info!("Lumys OS embedded server stopped");
        }
    }
}
...
pub fn start_server() -> Result<ServerHandle, Box<dyn std::error::Error>> {
    ...
    // Without this, API keys stored in ~/.lumys/.env are invisible to
    // the kernel's provider detection and credential resolver.
    load_dotenv_files();

    // Boot kernel (sync — no tokio needed)
    let kernel = LumysKernel::boot(None)?;
    ...
    info!("Lumys OS embedded server bound to http://127.0.0.1:{port}");
...
async fn run_embedded_server(
    ...
    info!("Lumys OS embedded server listening on http://{listen_addr}");
...
/// Load ~/.lumys/.env and ~/.lumys/secrets.env into the process environment.
/// System env vars take priority — existing vars are NOT overridden.
fn load_dotenv_files() {
    let home = if let Ok(h) = std::env::var("LUMYS_HOME") {
        ...
        std::path::PathBuf::from(user_home).join(".lumys")
    };
