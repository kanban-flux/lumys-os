// Lumys OS Agents Page — Multi-step spawn wizard, detail view with tabs, file editor, personality presets
'use strict';
...
function agentsPage() {
  return {
    ...
    async loadSpawnProfiles() {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/profiles');
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          Lumys OSAPI.get('/api/templates'),
          Lumys OSAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    async showDetail(agent) {
      ...
      try {
        var full = await Lumys OSAPI.get('/api/agents/' + agent.id);
    ...
    killAgent(agent) {
      var self = this;
      Lumys OSToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
          await Lumys OSAPI.del('/api/agents/' + agent.id);
          Lumys OSToast.success('Agent "' + agent.name + '" stopped');
          self.showDetailModal = false;
          await Alpine.store('app').refreshAgents();
        } catch(e) {
          Lumys OSToast.error('Failed to stop agent: ' + e.message);
        }
      });
    },
    ...
    killAllAgents() {
      ...
      Lumys OSToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        for (var i = 0; i < list.length; i++) {
          try {
            await Lumys OSAPI.del('/api/agents/' + list[i].id);
          } catch(e) { errors.push(list[i].name + ': ' + e.message); }
        }
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          Lumys OSToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
          Lumys OSToast.success(list.length + ' agent(s) stopped');
        }
      });
    },
    ...
    async openSpawnWizard() {
      ...
      try {
        var results = await Promise.all([
          Lumys OSAPI.get('/api/status').catch(function() { return {}; }),
          Lumys OSAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    nextStep() {
      if (this.spawnStep === 1 && !this.spawnForm.name.trim()) {
        Lumys OSToast.warn('Please enter an agent name');
        return;
      }
    ...
    async setMode(agent, mode) {
      try {
        await Lumys OSAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
        Lumys OSToast.success('Mode set to ' + mode);
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        Lumys OSToast.error('Failed to set mode: ' + e.message);
      }
    },
    ...
    async spawnAgent() {
      ...
      if (!toml.trim()) {
        this.spawning = false;
        Lumys OSToast.warn('Manifest is empty \u2014 enter agent config first');
        return;
      }

      try {
        var res = await Lumys OSAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            Lumys OSAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            Lumys OSAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
          ...
          this.spawnToml = '';
          this.spawnStep = 1;
          Lumys OSToast.success('Agent "' + (res.name || 'new') + '" spawned');
          await Alpine.store('app').refreshAgents();
          this.chatWithAgent({ id: res.agent_id, name: res.name, model_provider: '?', model_name: '?' });
        } else {
          Lumys OSToast.error('Spawn failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        Lumys OSToast.error('Failed to spawn agent: ' + e.message);
      }
      this.spawning = false;
    },
    ...
    async loadAgentFiles() {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/agents/' + this.detailAgent.id + '/files');
        this.agentFiles = data.files || [];
      } catch(e) {
        this.agentFiles = [];
        Lumys OSToast.error('Failed to load files: ' + e.message);
      }
    ...
    async openFile(file) {
      ...
      try {
        var data = await Lumys OSAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
        this.fileContent = data.content || '';
      } catch(e) {
        Lumys OSToast.error('Failed to read file: ' + e.message);
      }
    },
    ...
    async saveFile() {
      ...
      try {
        await Lumys OSAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
        Lumys OSToast.success(this.editingFile + ' saved');
        await this.loadAgentFiles();
      } catch(e) {
        Lumys OSToast.error('Failed to save file: ' + e.message);
      }
    ...
    async saveConfig() {
      ...
      try {
        await Lumys OSAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
        Lumys OSToast.success('Config updated');
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        Lumys OSToast.error('Failed to save config: ' + e.message);
      }
    ...
    async cloneAgent(agent) {
      ...
      try {
        var res = await Lumys OSAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
          Lumys OSToast.success('Cloned as "' + res.name + '"');
          await Alpine.store('app').refreshAgents();
          this.showDetailModal = false;
        }
      } catch(e) {
        Lumys OSToast.error('Clone failed: ' + e.message);
      }
    },
    ...
    async spawnFromTemplate(name) {
      try {
        var data = await Lumys OSAPI.get('/api/templates/' + encodeURIComponent(name));
        if (data.manifest_toml) {
          var res = await Lumys OSAPI.post('/api/agents', { manifest_toml: data.manifest_toml });
          if (res.agent_id) {
            Lumys OSToast.success('Agent "' + (res.name || name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
            this.chatWithAgent({ id: res.agent_id, name: res.name || name, model_provider: '?', model_name: '?' });
          }
        }
      } catch(e) {
        Lumys OSToast.error('Failed to spawn from template: ' + e.message);
      }
    },
    ...
    async clearHistory(agent) {
      var self = this;
      Lumys OSToast.confirm('Clear History', 'Clear all conversation history for "' + agent.name + '"? This cannot be undone.', async function() {
        try {
          await Lumys OSAPI.del('/api/agents/' + agent.id + '/history');
          Lumys OSToast.success('History cleared for "' + agent.name + '"');
        } catch(e) {
          Lumys OSToast.error('Failed to clear history: ' + e.message);
        }
      });
    },
    ...
    async changeModel() {
      ...
      try {
        var resp = await Lumys OSAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: this.newModelValue.trim() });
        var providerInfo = (resp && resp.provider) ? ' (provider: ' + resp.provider + ')' : '';
        Lumys OSToast.success('Model changed' + providerInfo + ' (memory reset)');
        ...
        Lumys OSToast.error('Failed to change model: ' + e.message);
      }
    ...
    async changeProvider() {
      ...
      try {
        ...
        var resp = await Lumys OSAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: combined });
        Lumys OSToast.success('Provider changed to ' + (resp && resp.provider ? resp.provider : this.newProviderValue.trim()));
        ...
        Lumys OSToast.error('Failed to change provider: ' + e.message);
      }
      this.modelSaving = false;
    },
    ...
    async addFallback() {
      ...
      try {
        await Lumys OSAPI.patch('/api/agents/' + this.detailAgent.id + '/config', {
          fallback_models: this.detailAgent._fallbacks
        });
        Lumys OSToast.success('Fallback added: ' + provider + '/' + model);
      } catch(e) {
        Lumys OSToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async removeFallback(idx) {
      ...
      try {
        ...
        Lumys OSToast.success('Fallback removed');
      } catch(e) {
        Lumys OSToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async loadToolFilters() {
      ...
      try {
        this.toolFilters = await Lumys OSAPI.get('/api/agents/' + this.detailAgent.id + '/tools');
      } catch(e) {
    ...
    async saveToolFilters() {
      if (!this.detailAgent) return;
      try {
        await Lumys OSAPI.put('/api/agents/' + this.detailAgent.id + '/tools', this.toolFilters);
      } catch(e) {
        Lumys OSToast.error('Failed to update tool filters: ' + e.message);
      }
    },
    ...
    async spawnBuiltin(t) {
      ...
      try {
        var res = await Lumys OSAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          Lumys OSToast.success('Agent "' + t.name + '" spawned');
          await Alpine.store('app').refreshAgents();
